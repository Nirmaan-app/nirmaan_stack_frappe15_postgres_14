...
**Owner:** Internal team.
**Last updated:** 2026-06-08 (Slice C-v2d-fix: per-area edit guard now validates the sent `area` against the SHEET'S defined areas [`sheet_config.area_dimensions` via new `_get_sheet_area_dimensions` helper, sheet_name VERBATIM] instead of THIS row's existing dict keys -- a defined-but-empty area is a valid value-edit target so the dict key is CREATED if absent [both flat qty/amount and nested rate paths; from=None for a new key]; an UNDEFINED area -> "not a defined area for this sheet", a sheet with no area_dimensions -> "no defined areas" [fail loudly, no permissive fallback]; blank -> 0.0 key-stays, provenance, structured edit_log, rate_subkey legality all UNCHANGED from C-v2d; backend-only [frontend already sends the correct verbatim area]; live-cert finding: BOQ-26-00145 'HVAC ' row_index 2 has qty_by_area={} but Phase 1/Phase 2 are defined sheet areas; test_review_screen 105->110 +5 [measured]; tsc 0 wizard errors in-container; Vite build exit 0 in-container; RE-LIVE-CERT OWED -- Nitesh restarts bench + re-tests the HVAC qty edit; rate-editing live-cert still owed against a Pattern-2-rate vehicle) // prior: Slice C-v2d (feat cd2cc156): per-area numeric editing [qty/amount/rate] via an EXTENDED `save_review_edit`)
...
**Latest commit:** feat 7fee7481 (Slice C-v2d-fix) // prior: feat cd2cc156 (Slice C-v2d)
