...
**Latest commit:** f24ac4fe feat(boq): SheetConfigPanel Section 3 -- column-role list + {role,area} save shape
...
- **Owner:** SheetSpokePage. Seeded once from `draft.sheet_config.column_role_map` when the doc first arrives (mirrors SheetConfigPanel's `initialized` guard -- `setRoleMapInitialized(true)` is inside the non-null guard so mutate() re-fetches don't overwrite user edits).
- **SheetConfigPanel:** receives `columnRoleMap` + `setColumnRoleMap` + `rows` as props. `handleSave` now explicitly writes `column_role_map` from the lifted prop (serialized back to `Record<string,string>` for the backend -- `area` is wizard-only state). The `...existing` spread still preserves all other unknown blob keys. `setColumnRoleMap` and `rows` are accepted in the interface for Slice 3d-ii forward compat but not yet consumed in the render.
...
- Backend blob stores `column_role_map: Record<string, string>` (role only). The `area` field is wizard-internal and never persisted. Conversion: seed (string → `{role, area:null}`) in SheetSpokePage; serialize (`{role, area}` → role string) in SheetConfigPanel.handleSave.
...
**Singleton roles (12):** `sl_no`, `description`, `unit`, `qty_total`, `rate_supply`, `rate_install`, `rate_combined`, `amount_total`, `amount_combined`, `make_model`, `row_notes`, `reference_images`. Enforcement: disabled in other rows' role dropdowns when already used. `usedSingletons` Map (role → col) is derived from `columnRoleMap` via useMemo.
