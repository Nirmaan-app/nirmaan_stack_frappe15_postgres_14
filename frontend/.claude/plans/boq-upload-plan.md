...
`boqTypes.ts`, `PricingGrid.test.ts`.
- **CONSUMED CONTRACT (live from 4a-BE):** `get_priced_rows` rows carry `row["remark"]` (string|null) +
  `row["color_by_cell"]` ({col_letter: token}, absent = none); endpoints `save_row_remark` / `save_cell_color` (blank
  clears; remark cap 250; a whole-row color = N save_cell_color calls). `PricedRow` gains `remark?` + `color_by_cell?`;
  NEW `COLOR_TOKENS`/`ColorToken` (8 tokens) + `RemarkSaveArgs`/`ColorSaveArgs` (boqTypes.ts, additive).
- **READ-ONLY GATING REUSE (the single root signal).** The grid's editability is the PRESENCE of a save callback
  (onSaveRate). The page passes `onSaveRemark`/`onSaveColor` ONLY when `!locked` (`locked ? undefined : handle`, exactly
  mirroring onSaveRate). When withheld (locked / taken-over), the grid renders remarks/colors READ-ONLY (no popover, no
  picker). NO second editability signal invented.
- **(1) REMARKS column (`PricingGrid.tsx`).** A trailing `<th>Remarks</th>` + per-row `<td>` AFTER the descriptor map
  (the established trailing-column pattern). A `RemarkCell` component: click-to-open a shadcn Popover with a `<Textarea>`
  (rows 3), a live 250 counter, Save + Clear (own draft/saving/error state, seeded on open, mutate-only refresh via the
  page) -- mirrors the review-screen remark idiom (NOT imported). READ-ONLY (no onSave) -> plain stored text.
  **EXCLUDED FROM THE KEYBOARD MATRIX** (a remark is multi-line; "Enter = move down" conflicts): NOT registered in
  cellRefs, `colCount`/`nextCell` UNCHANGED (the existing nextCell tests stay green). The trigger button
  `stopPropagation`s keydowns so the grid's table-level nav handler never hijacks it; the Popover content is portaled.
- **(2) COLOR fill (`PricingGrid.tsx`).** Per-descriptor-cell affordance: a tiny corner `ColorPicker` trigger (the td
  is now `relative`) opening an 8-swatch palette + "Apply to whole row" checkbox + "Clear color". The applied color
  renders on a **SEPARATE VISUAL CHANNEL -- a thick LEFT BORDER** (`colorClassForToken` -> `border-l-4 border-l-<c>`),
  DELIBERATELY not a background: the system owns the cell BACKGROUND (emerald priced / amber priced-non-priceable) + the
  dot + the blue inset focus ring, so a colored cell that is ALSO priced/active shows BOTH (border + fill + dot + ring,
  no masking). Picking calls `onApply(token, wholeRow)`; the grid maps to a `ColorSaveArgs[]` (1 cell, or
  `rowColorCells(displayDescriptors)` = every descriptor col for apply-to-row); the PAGE owns the N POSTs + ONE mutate
  (`handleSaveColor`). Clear sends `color:""`. READ-ONLY -> border shown, no trigger.
- **(3) PAGE handlers (`SheetPricingPage.tsx`).** `handleSaveRemark` / `handleSaveColor` mirror `handleSaveRate`:
  in-flight count (drives the Saving chip), POST(s) -> `mutate()` (remark/color re-derive authoritatively), client-clock
  lastSavedAt, and the SAME `isTakeoverError(BOQ_PRICING_LOCKED)` detection (an annotation save rejected by a held lock
  flips the page read-only + takeover banner, consistent with rates). The rate save path (commitRate/onSaveRate) is
  UNCHANGED -- annotations are a PARALLEL write.
- **(4) MINIMAL REVIEW-LIST (`SheetPricingPage.tsx`).** A "Review (N)" header toggle opens a collapsible strip ABOVE
  the grid (mirrors the Summary mount) listing rows with a remark; each entry click-jumps via a NEW
  `PricingGridHandle.scrollToRow(excelRow)` (resolves excel_row -> array index via rowsRef, focuses+centres the row's
  col-0 cell). Entries are a GENERIC `{kind, excelRow, description, text}` shape derived page-side (no new fetch), so the
  4b flag layer pushes computed flags into the SAME list. (Jump IS built -- not deferred.)
- **NEW pure exported helpers (unit-tested):** `colorClassForToken` (token->border class; "" fail-safe for unknown/
  null), `swatchClassForToken` (token->bg swatch; ""), `rowColorCells(displayDescriptors)` (the apply-to-row target
  cols -- NOTE: takes only the descriptors, not `row`, since the targets are row-independent -- a deviation from the
  prompt's `(row, displayDescriptors)` sketch), `remarkPreview` (trim + ellipsis; "" for blank).
- **VERIFIED:** Vitest **72 -> 80** (+8 in PricingGrid.test.ts: colorClassForToken [8 distinct + fail-safe],
  swatchClassForToken, rowColorCells, remarkPreview; the existing nextCell/nav tests UNCHANGED + green). tsc **3178**
  (== baseline), **0** in touched files (filtered SheetPricingPage|PricingGrid|boqTypes). Vite build exit 0 (PWA 168
  entries). **Manual live-cert pending Nitesh:** on a committed DATA sheet -- a trailing Remarks column with
  click-to-edit + 250 counter; a per-cell color picker (border channel, apply-to-cell + apply-to-row + clear) that
  coexists with the emerald/amber priced markers; the Review (N) strip lists remarked rows and click-jumps; on a
  general-specs (grid-only) sheet NONE of this shows (SheetDataGrid path); when the sheet is locked by another user the
  remarks/colors are visible but read-only. **NEXT = 4b** (the computed flag layer -- pushes flags into the SAME
  review-list seam) + the F1-F4 formula builder. Full detail in frontend/CLAUDE.md "Slice 4a-FE".

PRIOR: Slice 4a-BE -- ANNOTATION BACKEND (REMARKS + COLOR) (2026-06-22, feat pending, branch
feature/boq-phase-5). The backend half of Slice 4a: two USER-AUTHORED annotation layers on the pricing editor, stored
ADDITIVELY on top of the committed tier exactly like `BoQ Cell Pricing` (anchored to the durable Excel address +
committed version, with their own freeze-and-supersede lifecycle) -- but carrying NO rate / priceability / lock-identity
semantics, so they are TWO SEPARATE doctypes (NOT folded onto BoQ Cell Pricing, NOT one merged doctype). The FRONTEND
(trailing remarks column, the 8-color picker, the review-list feed) is the SEPARATE 4a-FE slice.
- **LOCKED SCOPE (owner-settled): DATA SHEETS ONLY.** General-specs (grid-only) sheets are read-only reference and get
  NO annotation -- only the node-based `get_priced_rows` path is in scope. `get_committed_sheet_grid` / `SheetDataGrid`
  are UNTOUCHED. Committed tier is NEVER mutated (§0); both new doctypes are additive, istable=0; `BOQ Nodes.remarks`
  (committed-tier provenance) is NOT written.
- **(1) NEW doctype `BoQ Cell Remark` (per-ROW).** autoname `BRMK-.YY.-.#####`, istable:0, track_changes:1, mirrors
  the BoQ Cell Pricing house style + its 10-role permission block. IDENTITY = (boq, sheet_name [VERBATIM #152],
  excel_row, committed_version) -- NO col_letter (a remark is per-row). VALUE `remark` (Small Text). GUARD `description`
  (Small Text -- the copy-forward MATCH GUARD for the future Slice-7 copy-forward, exactly like
  BoQ Cell Pricing.description: NOT part of the key, never branched on this slice). LIFECYCLE triple
  `remark_version`/`is_current`/`remarked_at`.
- **(2) NEW doctype `BoQ Cell Color` (per-CELL).** autoname `BCLR-.YY.-.#####`. IDENTITY = (boq, sheet_name, excel_row,
  col_letter, committed_version). VALUE `color` (Select, reqd) -- EXACTLY 8 stable string TOKENS (NOT hex; the frontend
  maps token->swatch): `red`, `orange`, `yellow`, `green`, `blue`, `purple`, `pink`, `grey`. GUARD `description`.
  LIFECYCLE triple `color_version`/`is_current`/`colored_at`. A whole-ROW apply is N cell records (the frontend fans
  out; the endpoint takes ONE cell). Both controllers are bare stubs (`pass`); the one-current invariant is
  ENDPOINT-enforced (pricing.py), NOT in the controller (mirrors the pricing/committed convention).
- **(3) `pricing.py` -- two write endpoints.** `save_row_remark(boq_name, sheet_name, excel_row, committed_version,
  remark, description=None)` + `save_cell_color(boq_name, sheet_name, excel_row, col_letter, committed_version, color,
  description=None)`, both `@frappe.whitelist(methods=["POST"])`. Each: (a) RESOLVES the committed ROW via the SAME
  row-level `_resolve_committed_cell` save_cell_price uses -- it keys on the node's `source_row_number` (NOT col_letter)
  and, crucially, does NOT gate on node_type (the 3e priceability guard is INLINE in save_cell_price only), so reusing
  it imposes NO priceability gate. **So a color (and a remark) is allowed on ANY committed cell -- non-priceable,
  zero-rate, anything -- where a PRICE would be rejected** (a test proves this contrast). (b) ACQUIRES/REFRESHES the
  single-editor lock (`acquire_or_refresh(boq, sheet, committed_version, frappe.session.user, now_datetime())`) AFTER
  the resolve, BEFORE any freeze/insert -- a lock reject (held fresh by another) mutates NOTHING. (c) Freeze-and-
  supersede upsert (freeze prior `is_current` via `set_value`, insert new at version max+1) + one `frappe.db.commit()`.
  VALIDATION: remark cap 250 (`_REMARK_MAX_LEN`, mirrors review_screen) -> throw; color must be one of the 8 tokens
  (`_COLOR_TOKENS`) -> throw. **CLEAR semantics:** a blank/whitespace remark OR color FREEZES the prior current and
  inserts NO new current, so the row/cell then has no `is_current` record and reads as "no remark"/"no color" (it will
  not appear in the future review-list). Returns `{ok, name, <version>, is_current, froze_prior, cleared}`.
- **(4) `pricing.py` -- two reads + the get_priced_rows merge.** `get_sheet_remarks` / `get_sheet_colors` (bare
  `@frappe.whitelist()`, GET-capable, mirror get_sheet_pricing) return the current set. `get_priced_rows` builds a
  remark index (excel_row -> text) + a color index (excel_row -> {col_letter: token}) ONCE before the per-row loop (no
  per-row query), then stamps `row["remark"]` (None when absent) + `row["color_by_cell"]` ({col_letter: token}, present
  ONLY on rows that have a color). The pre-existing "nothing to merge" early-return now also accounts for
  remarks/colors (not just prices), so an annotation-only sheet (no prices) still merges.
- **TESTS:** `test_pricing` 47 -> **63** (+16). `TestRowRemark` (7): save/v1 (+ guard carried), freeze-and-supersede/v2,
  clear-reads-as-no-remark, 250-ok/251-throw, non-existent-row-throw, lock-held-by-other-rejects-and-mutates-nothing,
  get_priced_rows surfaces the remark (a no-remark row reads None). `TestCellColor` (9): save/v1, freeze/v2,
  all-8-tokens-accepted, invalid-9th-throws, **COLOR-ON-A-NON-PRICEABLE-CELL** (the contrast: a price on the same Other
  row is rejected, a color is accepted -- adds its own `Other` node at source_row 50), clear-no-current,
  non-existent-row-throw, lock-reject-mutates-nothing, get_priced_rows surfaces color_by_cell. All prior 47 green (no
  regression). `bench migrate` CLEAN -- `BoQ Cell Remark` 20 columns / `BoQ Cell Color` 21 columns, both istable 0,
  verified via information_schema.
- **NEXT = Slice 4a-FE** (frontend): the trailing remarks column, the 8-color cell picker (token->swatch, row-apply
  fans out to N save_cell_color calls), the review-list feed -- consuming `row["remark"]` + `row["color_by_cell"]` +
  the `save_row_remark` / `save_cell_color` endpoints. The FE prompt must know the exact merged keys + the two endpoint
  signatures (above).

PRIOR: Slice 3e -- PRICEABILITY GATE + PER-SHEET OVERRIDE TOGGLE (2026-06-21, feat pending, branch
...
stale-lock-on-switch = no-op. NO sheet close/reorder, no "new sheet", no cross-BoQ tabs, no keyboard tab-cycling.
- **BACKEND `get_committed_state` (commit_gate.py) -- ADDITIVE sheet_order + ordering.** `sheet_order` is NOT a field on
  the queried `BoQ Committed Sheet Grid` tier; it lives on the committed `BoQ Sheet` tier (the per-sheet config snapshot
  the commit pipeline writes for BOTH dispositions, carried from the draft). So it is SOURCED via a second lookup
  (`frappe.get_all("BoQ Sheet", {boq, is_current:1}, ["sheet_name","sheet_order"])` -> a `{sheet_name: sheet_order}`
  dict, joined on the committed sheet identity, sheet_name VERBATIM #152). Each committed_state entry gains
  `sheet_order` (None if no current BoQ Sheet matches -- defensive; in practice every committed sheet has one), and the
  result is `.sort`ed by `(sheet_order is None, sheet_order or 0, sheet_name)` -> workbook order, None last, name
  tiebreak. PURELY ADDITIVE: existing keys/shape unchanged; the HUB (maps committedMap by sheet_name + doesn't assume
  order) stays UNCHANGED (its TenderingDialog list is now in workbook order -- a free improvement). NO migrate
  (sheet_order already exists on BoQ Sheet; this is a query field-add). `CommittedSheetState` (boqTypes.ts) gains
  `sheet_order: number | null`.
- **THE KEY-REMOUNT SAFETY FIX (the load-bearing change, WHY it matters).** A pricing->pricing tab switch does NOT
  remount the page/grid today (same RR route element, no key), so `draftRates` + in-flight debounced saves would carry
  across while `rows` swap underneath -- a leftover draft keyed `${row_index}:${col}` could fire against the NEW sheet's
  same-indexed row (silent cross-sheet corruption). FIX: `key={sheetName}` (the VERBATIM param) on the `<PricingGrid>`
  mount -> a tab switch UNMOUNTS+REMOUNTS the grid -> the EXISTING flush-on-unmount (`debouncers.forEach(deb =>
  deb.flush())` in a `[]`-deps cleanup, recon-verified to fire on unmount) commits the OLD sheet's pending drafts TO THE
  OLD SHEET (old closures captured old boqId/sheetName/commitVersion), and the NEW sheet gets a CLEAN grid (empty
  draftRates/proposedRates). Flush-then-switch: nothing lost, nothing cross-contaminated, no warn-modal. **Invariant
  future slices MUST keep: the grid stays keyed on sheetName so a switch can't carry drafts across.**
- **PAGE PER-SHEET STATE RESET.** The PAGE does NOT remount on a pricing->pricing route change, so a
  `useEffect([sheetName])` resets the sheet-specific page state: `saveError -> null`, `lastSavedAt -> null`,
  `takenOver -> false`, `summaryOpen -> false`. `inFlight` is DELIBERATELY NOT reset (a flush-on-unmount save from the
  old grid increments/decrements it in a pair; a hard reset to 0 would underflow when that in-flight save's finally runs
  -- and "Saving..." on the new sheet during the flush is honest). `hasUnsaved` re-derives from the remounted grid's
  `onDirtyChange(false)` on mount. The existing `takenOver`-reset effect keyed on `pricedData` stays.
- **THE TAB STRIP (SheetPricingPage.tsx, shadcn Tabs).** Fetched IN THE PAGE via `useFrappeGetCall` on the SAME
  `get_committed_state` endpoint the hub uses (a light single get_all; engineering call -- stated). Ordered by the NEW
  pure exported helper `orderCommittedSheets` (in PricingGrid.tsx, the established home for this feature's unit-tested
  pure helpers). `<Tabs value={decodedSheetName}>` (active tab = current :sheetName VERBATIM #152) with one
  `<TabsTrigger value={s.sheet_name}>` per committed sheet (value = VERBATIM sheet_name, label = `s.sheet_name.trim()`
  for DISPLAY only). `onValueChange` navigates to the hub's EXACT target
  `/upload-boq/hub/${boqId}/pricing/${encodeURIComponent(val)}` (no-op guard on self). Placed below the lock banners,
  above the editor note. Single committed sheet -> a lone tab renders (you-are-here context; stated). List loads
  independently -- the strip simply doesn't render until it arrives (grid never waits).
...
- **VERIFIED:** backend test_commit_gate **18 -> 19** (+1 `TestGetCommittedStateOrdering`: seeds grid rows + matching
  current BoQ Sheet rows with sheet_order OUT of order -> asserts the shape includes sheet_order AND the result is
  ordered ascending). Vitest **60 -> 64 GREEN** (+4 `orderCommittedSheets`: ascending order; null-order last + name
  tiebreak; sheet_name VERBATIM incl. trailing-space #152; no-mutate). tsc **3178** (== baseline), **0** in touched
  files (`SheetPricingPage|PricingGrid|boqTypes|tabs.tsx`). Vite build exit 0. NO bench migrate. Manual live-cert
  pending Nitesh (multi-sheet): tabs list committed sheets in workbook order; clicking a tab opens that sheet; a pending
  typed-but-uncommitted rate on sheet A persists to A when you tab to B (flush-on-unmount), and B opens clean; the lock
  banner re-derives per sheet. Full detail in frontend/CLAUDE.md.

PRIOR: Single-Editor Lock -- Slice B (FRONTEND: read-only gating + holder banner + takeover) (2026-06-21, feat
