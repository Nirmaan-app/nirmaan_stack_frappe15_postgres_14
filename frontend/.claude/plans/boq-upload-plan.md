...
by appending the 4 synthetic multi-area fixture sheets specified by Nitesh:
- merged_header_v1.xlsx: ELECTRICAL & ELV BOQ
- merged_header_v2.xlsx: ELECTRICAL & ELV BOQ
- single_header_v1.xlsx: HVAC BOQ
- single_header_v2.xlsx: HVAC BOQ
...
Key findings from the expanded run:

- All 4 new fixtures produced FileNotFoundError (both modes, all 4 targets).
  The .xlsx files do not exist in tests/fixtures/. The TARGETS entries are
  registered; the fixtures need to be physically added before any parse data
  can be collected. No Tier A-merged firing, no hrc auto-detect bump, no
  src_present_unparsed data -- because no files exist to parse.
- No sum invariant violations. No crashes. Diagnostic ran cleanly to completion
  across all 15 entries.
- Existing 11 targets: numbers identical to Chore #2 snapshot across all 3
  spot-checked targets (Paytm HVAC: qty src_present_unparsed=150/150 baseline
  confirmed; Kohler Electrical: clean target confirmed; Electrical Unpriced:
  numbers unchanged). Parser and diagnostic logic both unchanged.
- Mode 1 vs Mode 2 delta: zero for all 11 parseable targets (consistent with
  Chore #2 finding -- the auto-detect / hrc delta is a parser-side gap, not a
  header-detection gap).

Implications for queue positions 4 and 5: The fixture-missing finding means the
v5.19 strategic re-evaluation chat (queue position 4) should address whether to
(a) create the 4 synthetic .xlsx fixtures now so empirical data can be
collected, or (b) proceed to Phase 1.9q investigation first (Paytm HVAC
text-coercion gap) using the existing 11-target corpus. No new empirical data
on the merged-header shape was generated by this chore -- that remains an open
empirical question.

Parser tests 375 unchanged --- agreement 27 strict.

Chore commit: 9d4abf36
