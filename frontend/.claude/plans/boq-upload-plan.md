...
**Owner:** Internal team.
**Last updated:** 2026-06-08 (Slice C-v2d: per-area numeric editing [qty/amount/rate] via an EXTENDED `save_review_edit` -- new optional `area` + `rate_subkey` params; flat one-hop qty_by_area/amount_by_area + nested two-hop rate_by_area [subkeys reused from parser `_RATE_ROLE_TO_KIND`]; bare-dict read-modify-write riding the existing `_RESAVE_LIST_JSON_FIELDS` handling [why EXTEND not new path]; blank -> 0.0 with the area key kept; structured edit_log keying [optional area+rate_subkey, old flat entries valid]; validation rejects nonexistent-area / missing-rate_subkey / illegal-rate_subkey / non-per-area-field-with-area; FRONTEND per-area edit block in ReviewTree mirroring the C-v2 confirm-dialog pattern + rate_by_area TS type fix flat->nested + Obs A edit_log latest-first; test_review_screen 105 [measured, was 95, +10], total wizard 276 [derived, full suite not re-run]; tsc 0 wizard errors in-container; Vite build exit 0 in-container; LIVE-CERT NOT YET DONE [rate-editing live-cert OWED against a Pattern-2-rate vehicle -- Electrical has no nested rate data])
...
**Latest commit:** feat cd2cc156 (Slice C-v2d)
