...
- Row navigation **branches on the REQUIRED `surface` prop (`"won" | "pre-won"`, feat: boq-row-click-by-surface)** -- the line above (`onClick` -> `navigate(/upload-boq/hub/${row.name})`) describes the PRE-WON branch only, and stopped being the whole truth at 8c0226f6. boqId = `row.name` (BOQs docname from Frappe autoname). Hub route: `upload-boq/hub/:boqId` (routesConfig.tsx).
  - `surface="won"` -> the READ-ONLY sheet viewer: `get_committed_state` resolves the first committed sheet (sheet_order, nulls last, then sheet_name) -> `/upload-boq/hub/:boq/view/:sheet`; nothing committed -> a `warning` toast and NO navigation; a failed read -> its own `destructive` toast, also no navigation.
  - `surface="pre-won"` -> straight to `/upload-boq/hub/:boq`. **No `get_committed_state` call, no spinner, no toast** -- that request exists only to find a landing SHEET, which the hub does not need.
  - ⚠️ The pre-Won branch is ALSO gated on `boqAccess.canOpenBoqHub` (DERIVED from `UPLOAD_BOQ_ACCESS`, so it cannot drift from `<RoleRoute allowed={UPLOAD_BOQ_ACCESS}>`). Viewing a pre-Won stub is NOT limited to the four wizard profiles, so an Accountant or Sales user can reach this list on one; without the gate the click would land them on Access Denied. They fall back to the preview path, which writes nothing.
  - ⚠️ `surface` is PASSED IN, never derived from `tendering_status` inside the component. `project.tsx` branches on `data.tendering_status && !== "Won"`, so a legacy row with a BLANK tendering_status renders the WON page while `isOperational("")` is false -- deriving it here would give that project pre-Won behaviour on the Won page.
  - The pencil is UNCHANGED on both surfaces (always the hub). On pre-Won it coincides with the row click for the four wizard profiles; for procurement / Project Lead there is no pencil and the row IS their route.
...
- BoqProjectTab is reused as the **BOQs tab** of `TenderingProjectView` (feat: tendering-project-tabs, 2026-06-16). Passed as `projectId={data.name}` where `data` is the Projects doc -- no transform. **No longer VERBATIM: it now also takes `surface="pre-won"` here vs `surface="won"` in `project.tsx`** (see the row-navigation bullet above), which is the ONE behavioural difference between the two mounts.
