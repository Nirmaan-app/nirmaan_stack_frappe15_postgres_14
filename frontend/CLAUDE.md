...
**Special:** `Administrator` user (user_id) has hardcoded Admin access. PMO Executive mirrors Admin access **except** TDS Approval (view-only, cannot approve/reject), Payment Approval (no Approve tab, no edit fulfilled), PR Approval (no "Approve PR" tab, blocked from the approve view even by direct URL — approvers are Admin + Project Lead, `PR_ADMIN_ROLES`), PO / Sent Back PO / PO Revision / WO / Amended WO approval (no Approve tabs, approve views guarded by direct URL too — `PO_ADMIN_ROLES`, `SR_ADMIN_ROLES`, 2026-09-17), payment settling (no "Payment need to paid" / "Reconciliation Pending" tabs, no Record Paid Entry, no Request Payment or payment-terms edit on a PO — 2026-09-17), vendor invoice approval (no Pending Invoice Approvals tab — `INVOICE_APPROVAL_PROFILES`, both sides, 2026-09-17), and PR-flow new-item creation (request-only like a Project Manager in `new_items="false"` categories — no category-restriction bypass). HR Executive has Admin Options sidebar access.
...
- **Rate-helper panel/strip/workings refinements (RM-3a, `RateHelperPanel.tsx` / `SheetPricingPage.tsx` /
  ...
  before (single-group suggestions stay backward-shaped)**. `pricingSheetHelper` emits two groups
  (`Cable — per Mtr` / `Termination — per Set`) on **EVERY** wiring row — **owner ruling 2026-08-22 REVERSED
  the original "flat on a termination row" shape**, because the row TEXT (`isTerminationRow`) was deciding
  which pipeline RAN and silently discarding the other rate. It now decides only which block is PRIMARY
  (whose finals become `values`), never which is COMPUTED. Groups stay DISPLAY-ONLY (the applied value is
  still `Suggestion.values`). **Memo shield untouched** — no new per-row grid prop,
  `pricingRowPropsAreEqual` unchanged.
  (4) **TWO STACKED HEADLINES (owner Rulings A + C, 2026-08-22):** `Suggestion.headlines?:
  {label, values}[]` — when a helper sets it the collapsed header renders one line PER ENTRY (label +
  figure, stacked, taller); **ABSENT ⇒ the single headline, byte-unchanged**, which is what keeps every other
  category identical. ⚠️ **THE ENTRIES ARE NEVER SUMMED**: cable is per **Mtr** and termination is per
  **Set**, so a total would apply a per-set rate across a metre quantity (row 194 of BOQ-26-00201 is 2,900 m).
...
- **CSV Export Pattern**: DataTable columns configure export through the column's **`meta`** object — **`meta.exportValue`** (a `(row) => string` formatter), **`meta.exportHeaderName`** (custom column name) and **`meta.excludeFromExport`**, read by `src/utils/exportToCsv.ts`. Export respects current table sorting and column order. ⚠️ This line previously documented an **`exportMeta`** object with `header` / `value` / `exportFileName` keys — **that API does not exist and never did** (0 hits in `src/`, against 69 files using `meta.exportValue`). Corrected 2026-08-21 after a Snag List agent built against the documented name and found nothing to build on.
...
- **Critical PO Tasks — a PO's package comes from its PR's TAGS, not `work_package`**: the "Link New POs" / "Edit Task" pickers map a PO → PR → procurement package to filter the dropdown. Since the v3.0 `migrate_work_package_to_pr_tags` patch `Procurement Requests.work_package` only carries the PR type (`"Normal"` / `"Custom"`), so the old `pr.work_package === selectedPackage` comparison matched nothing and every real package showed **"Available POs (0)"**. The rule now lives in ONE pure helper — `filterPOsByPackage` (+ `buildPRPackageMap`) in `src/pages/projects/CriticalPOTasks/utils.tsx` — consumed by both `LinkPODialog.tsx` and `EditTaskDialog.tsx` (ADR-0010 F1/F4); do not re-derive it in a third place. `useCriticalPOProcurementRequests` fetches the child field as ``"`tabPR Tag Child Table`.tag_package as tag_package"`` — a **LEFT JOIN**, so a PR with two tags comes back as **two rows** (fold with `buildPRPackageMap`) and an untagged PR comes back once with `tag_package: null`. The `"Custom"` dropdown entry is **not** a Procurement Package: it keeps custom-flow PRs *and* every PO whose package cannot be resolved (untagged PR, or no PR at all) reachable. Selecting the child field needs a cast — it is valid for Frappe's list API but sits outside the SDK's `keyof`-based `fields` type. See root `../CLAUDE.md` Domain Gotchas + `../.claude/context/domain/procurement.md`.
- **Dialogs that mutate their own list must hold the row's NAME, not the row object**: `CriticalPOTasksList.tsx` used to keep `useState<CriticalPOTask>` for the open Edit dialog, so linking/unlinking a PO called `mutate()`, refreshed the table, and left the still-open dialog rendering the snapshot taken when it opened (stale PO chips). Store `editingTaskName` and derive the live row from the refetched list (`useMemo` over `tasks`). Same shape applies to any controlled row dialog whose actions refetch the parent list.
- **Critical PO links: read `task.linked_pos`, write ONLY through `useUpdatePOTaskLinks`**: links live on the PO as `Critical PO Task Child Table` rows, not in `Critical PO Tasks.associated_pos` (still in the DB, unused — never read it or `updateDoc` it). **Read:** `useProjectPOTaskLinks(projectId)` / `useAllPOTaskLinks()` (`src/pages/projects/data/critical-po/useCriticalPOQueries.ts`) fetch child rows through the Procurement Orders parent join (``"`tabCritical PO Task Child Table`.critical_po_task as critical_po_task"`` — same `keyof` cast as the PR-tag join), and `buildTaskPOMap` / `attachLinkedPOs` (`CriticalPOTasks/utils.tsx`) put a sorted `linked_pos: string[]` on each task. The tracker's PO badge shows the stored `linked_po_count`; clicking it lists `linked_pos`. **Write:** `useUpdatePOTaskLinks().updateLinks(projectId, { add, remove })` → `update_po_task_links` (atomic; a "Change" is ONE call carrying both `remove` and `add`), which invalidates the link + task SWR caches — used by the tracker dialogs, `LinkedCriticalPOTag` and the dispatch hook `useCriticalPOTaskLinking`. The material-plan API returns `linked_pos` / `linked_pos_count` (renamed from `associated_pos*`). PO Summary's Critical PO filter is a server-side column facet on the child field (column `id: "Critical PO Task Child Table.task_name"`, `meta.facet.field: "task_name"`; `get_facet_values` auto-detects child-table fields); the old hand-built filter bar is gone. See root `../CLAUDE.md` Domain Gotchas.
