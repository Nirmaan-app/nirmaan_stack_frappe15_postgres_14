...
**Download priced tender conventions (Phase 5 Slice 5b -- `PricedTenderDialog.tsx` + `downloadBlob.ts` + `BoqHubPage.tsx` + `SheetCard.tsx` + `boqTypes.ts`; full detail: plan section "Slice 5b"):** the hub UI for the 5a Excel write-back. **DELIBERATELY DISTINCT from "Export Finalized"** (`ExportWorkbookDialog`, the D2b fresh review .xlsx built client-side from review rows) -- this DOWNLOADS THE ORIGINAL TENDER FILE with the priced rates + the user's colour/remark notes stamped in SERVER-SIDE (the 5a `export_priced_workbook` endpoint). Two buttons must never read as duplicates.
