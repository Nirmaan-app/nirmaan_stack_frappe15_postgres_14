...
- **columnRoleMap owned by SheetSpokePage.** `useState<Record<string, ColumnRoleEntry>>({})` with seeding effect mirrors the SheetConfigPanel `initialized` guard: `setRoleMapInitialized(true)` fires only inside the `draft?.sheet_config !== null` guard, so mutate() re-fetches don't overwrite user edits. Seeded from `draft.sheet_config.column_role_map` as `{ role, area: null }` entries.
- **ColumnRoleEntry type in boqTypes.ts.** `{ role: string; area: string | null }`. Backend blob uses `Record<string, string>` (role only); `area` is wizard-only. Convert on seed (string → `{role, area:null}`) and on save (serialize back to role string in SheetConfigPanel.handleSave via `Object.fromEntries(...map(([col, entry]) => [col, entry.role]))`).
...
- **Singleton roles (12):** `sl_no`, `description`, `unit`, `qty_total`, `rate_supply`, `rate_install`, `rate_combined`, `amount_total`, `amount_combined`, `make_model`, `row_notes`, `reference_images`. Enforced by disabling in other rows' role dropdowns (not a runtime error). `usedSingletons` Map (role → col) derived via useMemo from `columnRoleMap`.
