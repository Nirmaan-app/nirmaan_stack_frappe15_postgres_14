...
**Status (2026-06-08, updated — Restructure Slice 1a landed):** Restructure surface Slice 1a (searchable sheet-view component + temporary dev route) landed FRONTEND-ONLY; tsc + Vite green in-container, live-cert OWED (see end of this Status block). Prior: Slice C-classification read-only recon ran 2026-06-08; findings pasted to chat-Claude for analysis before build prompt (NO code change). UNIFY-ON-(-1) + Slice A review-screen backend COMPLETE (feat fff26abd; BACKEND ONLY). Slice B1 review-screen spine COMPLETE (feat 0683f7b9; SheetReviewPage + ReviewTree + get_structural_breaks endpoint + 3 tests). Slice B1.1a COMPLETE (feat 58d2ed44; BACKEND ONLY -- get_review_rows extended with column_descriptors; 50 total review-screen tests, 218 total wizard tests). Slice B1.1b-i COMPLETE (feat 3e846ba1; FRONTEND ONLY -- descriptor-driven column layer + ClassificationPill; ColumnDescriptor type; anchor letters; tsc 0 wizard errors; Vite build exit 0). Slice B1.1b-fix-A COMPLETE (BACKEND ONLY -- flatten_resolved_row writes human_parent=-1; agreement #54 unify completed; 63 parse-run tests, 221 total wizard tests). Slice B1.1b-fix-B COMPLETE (FRONTEND ONLY -- ReviewTree.tsx: parent column + scroll-to-parent, pill-no-clip, ancestor-only collapse, pill-above-text; tsc 0 wizard errors; Vite build exit 0). Slice B1.1b-ii COMPLETE (FRONTEND ONLY -- column-subset selector Popover + Checkbox + three independent spacer/note/subtotal visibility toggles; children-of-hidden-annotation render at original depth; tsc 0 wizard errors; Vite build exit 0). Slice B1.1b-iii COMPLETE (FRONTEND ONLY -- Description cell split: Classification fixed anchor [chevron+pill, flat-left, w-36] + text-only Description with depth indent; tsc 0 wizard errors; Vite build exit 0). Slice B2a COMPLETE (BACKEND + FRONTEND -- advisory flags: four §6.5.3 heuristics; get_structural_breaks extended returns flags; ReviewTree Info marker + click-to-reveal; 76 review-screen tests +26; 247 total wizard tests; tsc 0 wizard errors; Vite build exit 0). Slice B2a single-source fix COMPLETE (refactor d9fa6b69 -- flags folded into get_review_rows; client-side computeAdvisoryFlags deleted ~87 lines; AdvisoryFlag moved to boqTypes expanded to full backend shape; SheetReviewPage forwards flags prop; 77 review-screen tests +1 wiring; 248 total wizard tests; tsc 0 wizard errors; Vite build exit 0). Slice B2a-fix COMPLETE (BACKEND + FRONTEND -- flag-(ii) tightened: rate-AND-zero-amount only, qty trigger dropped, new reason text; ReviewTree single-open accordion + master show/hide-all-notes toggle + dismiss-on-table-click scoped to <table>; SheetReviewPage per-category flag count summary strip; 78 review-screen tests +1; 249 total wizard tests; tsc 0 wizard errors; Vite build exit 0). Slice B2a-fix wording COMPLETE (FRONTEND text-only -- toggle "notes"→"flags", strip label "Advisory:"→"Flags:"; tsc 0 wizard errors; Vite build exit 0). Slice B2b COMPLETE (FRONTEND ONLY -- left-gutter expander column [w-8, frozen-left sticky left-0 z-10 bg-background on body, top-0 left-0 z-30 bg-muted on corner], inline read-only detail panel [original-vs-effective classification+parent as Excel row#, value fields, flag reasons reuse, edit_log history, empty reason slot, provenance badge], Option-B mutual exclusivity [toggleDetailRow clears expandedFlagRow; toggleFlagRow clears expandedDetailRow; table onClick clears both], totalCols 5→6, sticky-header fix [solid bg-muted on <th> cells, z-20; dropped semi-transparent bg-muted/50 from <tr>], ClassificationPill restyle [rounded-full, CLS_PILL_CLASSES soft per-type opaque fill, left-border dropped], aria-label "advisory notes"→"flags"; tsc 0 wizard errors; Vite build exit 0). Slice B2c COMPLETE (FRONTEND ONLY -- new "Status" fixed anchor column [after Excel Row, green "Edited" badge bg-green-100/dark:bg-green-900 / blank on original, not frozen-left], green row tint on edited rows [bg-green-50 dark:bg-green-950/30, highlight flash still wins via cn() order], provenance rule [edited_at !== null OR edit_log.length > 0], totalCols 6→7, detail panel edit-focused reshape [value-field block qty/rate/amount removed; grid-cols-2 sm:grid-cols-3 → grid-cols-2; comment updated; panel still has provenance+orig-vs-eff+flags+edit_log+reason slot], panel "edited" badge amber→green [bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300]; read-only infra -- all rows ORIGINAL until Slice C; tsc 0 wizard errors; Vite build exit 0). Slice B2c-fix COMPLETE (FRONTEND ONLY -- Status cell unedited branch null→muted gray "Original" pill [bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300]; green "Edited" pill + row tint unchanged; tsc 0 wizard errors; Vite build exit 0). Slice C-v1 COMPLETE (feat 2bf77d62; BACKEND delta + FRONTEND read-side render -- first of three C-values sub-slices; backend adds optional `reason` 6th edit_log key [uniform shape] + `edited_at` to save_review_edit return; FRONTEND: EditLogEntry.reason?:string added to boqTypes.ts; ReviewTree edit-history loop renders ` · reason: {entry.reason}` in muted styling when present; the row-level "Reason — (added in a later step)" placeholder slot is UNTOUCHED [that is the C-v2 current-reason slot, distinct from the per-entry history reason]; review_screen tests 78→84; tsc 0 wizard errors; no Vite build [no behavioral UI change to live-cert until C-v2 wires the input]). Slice C-v2 COMPLETE (feat aa74a023; FRONTEND ONLY -- value-editing now live for the 7 flat numeric fields [qty_total, rate_supply, rate_install, rate_combined, amount_total, amount_supply, amount_install; mirrors backend _VALUE_FIELDS]; editable rule `value_key === null && EDITABLE_VALUE_FIELDS.has(value_field)`; detail-panel inline Input type=number + Apply per editable descriptor; single confirm AlertDialog [one-line summary + OPTIONAL reason Input]; useFrappePostCall save_review_edit with sheet_name VERBATIM [#152 guard] + reason raw [backend normalizes blank->None]; onSaved -> SheetReviewPage handleSaved sets lastSavedAt [from returned edited_at] + mutate(); sheet-level "All changes saved" status anchor [reports, not batch-save]; header sub-line "Read-only review"->"Review & edit"; SaveReviewEditResponse type added to boqTypes; per-area cells + text fields read-only [C-v2b]; classification/parent edit later slices; the C-v1 per-entry history reason render + the per-ROW "Reason -- (added in a later step)" placeholder slot UNTOUCHED; tsc 0 wizard errors; Vite build exit 0 in container; live-cert is Nitesh's separate next step). Slice C-v2b COMPLETE (feat ae65555c; BACKEND + FRONTEND -- inline text-editing for unit + make_model; FRONTEND adds EDITABLE_TEXT_FIELDS [unit, make_model -- mirrors backend _TEXT_FIELDS] + editableTextDescriptors [same `value_key === null && Set.has(value_field)` gating as editableDescriptors, so a text input shows only when the sheet maps that column] + a SEPARATE "Edit text" detail-panel block [text Input type=text + Apply per text descriptor]; Apply saves DIRECTLY via saveTextField -> save_review_edit [NO confirm dialog, NO openValueConfirm -- silent save; the numeric confirm-dialog flow is UNTOUCHED, ADDITIVE not a refactor], sheet_name VERBATIM [#152], reuses onSaved(edited_at) so the row flips to "Edited"; textInputs state seeded alongside editInputs in the same detail-open useEffect [editableTextDescriptors added to deps]; the inline saveError moved OUT of the numeric block to a single shared render below both edit blocks so text-only sheets surface failures; description/row_notes stay read-only [NOT added to either editable Set]; backend adds _TEXT_FIELDS to _ALLOWED_EDIT_FIELDS + a string write-branch [setattr verbatim, NO float() coercion; blank string clears to None; numeric float-path + human-field path unchanged]; review_screen tests 84->90 +6; tsc 0 wizard errors; Vite build exit 0 in container; live-cert is Nitesh's separate next step). Slice C-v2c COMPLETE (feat da6bb6d1; BACKEND + FRONTEND -- per-row Remarks [human-only annotation, NOT a data edit]; backend new `remarks` Small Text field on BoQ Review Row [migrated; column landed as text] + NEW save_review_remark endpoint [writes ONLY remarks via frappe.db.set_value, NOT doc.save -- never edited_at/edit_log, so a remark-only row stays "Original"; 500-char hard guard; blank clears to None; sheet_name VERBATIM] + remarks added to get_review_rows all_fields; save_review_edit UNTOUCHED; FRONTEND ReviewRow.remarks type + separate save_review_remark useFrappePostCall [own loading flag] + blue MessageSquare marker in a right-aligned marker-group wrapper INSIDE the Classification cell next to the flag marker [NO new column, totalCols unchanged] gated `hasRemark && (showAllRemarks || expandedDetailRow===idx)` [panel-only -- click marker calls toggleDetailRow; NO inline reveal row] + master Show/Hide-all-remarks toggle [default OFF, mirrors flags toggle, blue] + detail-panel "Remarks" block [Textarea + live {n}/500 counter + Save disabled over-cap/unchanged/saving] with DEDICATED remarkError [separate from shared saveError so the two never cross-display] + onRemarkSaved prop [mutate WITHOUT advancing the "All changes saved" edit anchor]; SheetReviewPage wires onRemarkSaved + adds a sheet-level "Remarks: N" count strip mirroring the flags strip [omitted when zero]; review_screen tests 90->95 +5; tsc 0 errors; Vite build exit 0 in container; live-cert is Nitesh's separate next step). Slice C-v2c-polish COMPLETE (fix ae9dcff2; FRONTEND + cap -- live-cert UX follow-up: (1) remark marker ALWAYS-VISIBLE [`remarkMarkerShown = hasRemark`; the `showAllRemarks || expandedDetailRow===idx` gate is GONE -- a marker advertises the remark; click still opens the panel via toggleDetailRow]; (2) master "Show/Hide all remarks" toggle REMOVED entirely [`showAllRemarks` state + `toggleShowAllRemarks` + `hasRemarksAny` + the controls-bar button deleted -- this SUPERSEDES the C-v2c "master Show/Hide-all-remarks toggle [default OFF...]" line above]; (3) fixed-width panel inputs across all three detail-panel edit blocks [numeric "Edit values" + text "Edit text" switched `grid grid-cols-2 sm:grid-cols-3`->`flex flex-wrap` with `w-52` items; Remarks block capped at `max-w-md`] so inputs no longer stretch to the full table width [colSpan=totalCols] and the Apply/Save button sits beside/below -- no horizontal scroll on a many-column sheet; layout-only, no save logic/confirm-dialog/endpoint/dirty-rule change; (4) cap `REMARK_MAX_LEN` 500->250 [counter, over-cap message, Save-disable; backend `_REMARK_MAX_LEN` matches] -- read path unchanged so a stored >250 remark still displays, only re-saves past 250 are blocked; "Remarks: N" strip KEPT [SheetReviewPage's independent `remarkCount`, untouched]; SheetReviewPage.tsx needed NO change; the two backend cap tests retuned to 250/251; review_screen tests stay 95; tsc 0 wizard errors; Vite build exit 0 in container; NO doctype/schema change. NOT fixed [deliberate]: clicking the marker on an already-open panel toggles it closed). Slice C-v2d COMPLETE (feat cd2cc156; BACKEND + FRONTEND -- per-area numeric editing [qty/amount/rate]; the last C-values slice. BACKEND EXTENDS save_review_edit [new optional area + rate_subkey params; area=None == prior flat path unchanged]: FLAT one-hop qty_by_area/amount_by_area + NESTED two-hop rate_by_area [rate_subkey in supply_rate/install_rate/combined_rate, reused from parser _RATE_ROLE_TO_KIND]; bare-dict read-modify-write riding the existing _RESAVE_LIST_JSON_FIELDS handling [why EXTEND not a new endpoint]; blank -> 0.0 with the area key KEPT; STRUCTURED edit_log keying [optional area+rate_subkey, old flat entries valid]; validation rejects nonexistent-area / missing-rate_subkey / illegal-rate_subkey / non-per-area-field-with-area; stamps edited_at [row flips to "Edited"]. FRONTEND: ReviewTree new "Edit per-area values" detail-panel block [editableAreaDescriptors = displayDescriptors.filter(value_key !== null && EDITABLE_AREA_FIELDS.has(value_field)); Input type=number + Apply -> openAreaConfirm -> the SAME C-v2 confirm AlertDialog -> confirmValueSave sends area+rate_subkey], areaInputs state keyed by d.col [value_field repeats per area], flex flex-wrap + w-52 [C-v2c-polish width convention]; rate_by_area TS type fix flat Record<string,number> -> nested Record<string,RateByAreaCell> [new RateByAreaCell type; NO static read site broke -- resolveDescriptorValue walks generically]; EditLogEntry + SaveReviewEditResponse gain optional area/rate_subkey; Obs A edit_log history render reversed to LATEST-FIRST [[...edit_log].reverse() on a copy] + per-area target rendered inline in history line + confirm dialog [e.g. "rate_by_area (Zone A / combined_rate): 8 -> 9.5"]; review_screen tests 95->105 +10 [TestSaveReviewEditPerArea]; tsc 0 wizard errors in-container; Vite build exit 0 in-container; LIVE-CERT NOT YET DONE -- rate-editing live-cert OWED against a Pattern-2-rate vehicle [Electrical has no nested rate data], BOQ-26-00145 doubly-dirtied by probes [re-parse before live-cert]). Slice C-v2d-fix COMPLETE (feat 7fee7481; BACKEND-ONLY -- per-area edit guard now validates the sent area against the sheet's defined areas [sheet_config.area_dimensions via new _get_sheet_area_dimensions helper, sheet_name VERBATIM] instead of the row's existing dict keys, so a per-area value edit now works on a DEFINED-but-empty area [key created if absent; flat + nested rate paths]; UNDEFINED area / no-area_dimensions still rejected with clear messages; blank->0.0 key-stays + provenance + structured edit_log unchanged; NO frontend change [the frontend already sends the correct verbatim area]; test_review_screen 105->110 +5; tsc 0 wizard errors + Vite build exit 0 in-container; RE-LIVE-CERT OWED). Module 1b COMPLETE. Module 2b-i ✅ COMPLETE (feat 81568df9;
...
**Restructure surface Slice 1a -- SheetSearchView conventions (feat pending):**
...
- **TEMPORARY dev route (REMOVED in 1b):** `_DevSheetSearchHarness.tsx` + the `upload-boq/_dev-sheetview/:boqId/:sheetName` route in `routesConfig.tsx`, both carrying a "REMOVE in Slice 1b" comment. Not linked from any real UI. Live-cert URLs on BOQ-26-00145: `/upload-boq/_dev-sheetview/BOQ-26-00145/Fire%20Fitting` (clean), `.../Electrical%20` and `.../HVAC%20` (trailing-space). LIVE-CERT is Nitesh's separate step.
