...
**Section-grain confirmedFields keys (Slice 3e feat e60e768c):** SheetConfigPanel's `confirmedFields: Set<string>` includes three stable section-level keys alongside per-field keys: `"section:rows"`, `"section:areas"`, `"section:roles"`. A section is confirmed when: (a) any field in the section is interacted with (focus or change events both call `touchS1/touchS2/touchS3` which set both the field key AND the section key), (b) the user clicks "Accept all sections as-is" (sets all three at once), or (c) a Save or Mark-as-reviewed completes (all keys added to confirmedFields). Section h3 headings show the sparkle when their section key is unconfirmed. Layer 1 is satisfied when all three section keys are present.
