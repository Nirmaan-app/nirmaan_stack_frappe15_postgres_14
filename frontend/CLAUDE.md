...
`boqTypes.ts` / backend / SheetDataGrid UNTOUCHED.
...
- **FINDING 1 -- the trailing REMARKS cell now joins the keyboard matrix.** 4a-FE excluded it (a multi-line Textarea's
  ...
  0-4; `commitActiveRate` safely no-ops on the remarks cell since `displayDescriptors[length]` is undefined). (b) the
  remarks **`<td>` is the nav focus target** (`tdFocusProps(rowIdx, remarksColIndex)` + `cellNavClass` for the ring);
  arrows land + onFocus sets activeCell. (c) **Open-state LIFTED to the grid** (`openRemarkRowIdx: number|null`, array
  index) -> `RemarkCell` is now CONTROLLED (`open` + `onOpenChange` props; draft/saving/error stay LOCAL, seeded on the
  open transition via `useEffect([open])` so keyboard-open -- which sets the grid state directly, not via onOpenChange --
  still seeds). (d) **Enter opens the editor**: a new branch in `handleGridKeyDown` BEFORE the generic Enter->down --
  `if (activeCell.colIndex === remarksColIndex && e.key === "Enter" && onSaveRemark) { preventDefault; setOpenRemarkRowIdx;
  return; }`. A READ-ONLY remarks cell (no onSaveRemark) skips the branch -> Enter falls through to the generic
  Enter->down, matching every other read-only cell. (e) **Esc closes** (Textarea onKeyDown -> `onOpenChange(false)`; the
  grid's onOpenChange refocuses the cell via `focusCell`). (f) **Enter INSIDE the editor = save-and-move-down**: the
  Textarea onKeyDown Enter (no Shift) -> `commit(draft.trim(), true)` -> save, close, then `onMoveDown` =
  `nextCell({rowIndex,colIndex:remarksColIndex}, "down", ...) + focusCell` -- REUSES the matrix path a rate cell's Enter
  uses (not a reimplementation); Shift+Enter = newline (kept); on a save error the editor STAYS open. `onCloseAutoFocus`
  is `preventDefault`'d so the GRID (not Radix) governs focus on close; the trigger button is `tabIndex={-1}` (not a
  competing nav stop). **The existing rate-cell nav is byte-for-byte UNCHANGED.**
- **FINDING 2 -- apply-whole-row color intermittently lost (a frontend state/timing RACE).** Root cause (recon,
  ...
  checkbox only toggles `wholeRow`; an explicit **Apply** button (disabled until `armed !== null`) is the ONLY thing
  that saves, reading `{armed, wholeRow}` TOGETHER at click time via `submit(armed)`; a **Clear** button sends `""`.
  Apply/Clear close the popover; opening seeds `armed` from the cell's current color + resets `wholeRow`. **This kills
  the race by construction** -- no moment a half-set intent is sent. The grid's `onApply(token, wholeRow)` fan-out
  (`wholeRow ? rowColorCells(displayDescriptors) : [d.col]`) and the page's `handleSaveColor` (N POSTs + one mutate) are
  UNCHANGED -- only the picker's decide/submit changed (swatch-click-saves -> arm-then-Apply).
- **VERIFIED:** Vitest **80 -> 81** (+1; PricingGrid.test.ts 57 -> 58: a new nextCell test for the +1 column -- arrow-
  right lands on remarks, arrow-left returns, right-edge stop, Tab-wrap to next row, last-row Tab stop; the existing 4
  nextCell tests UNCHANGED + green, proving the rate-cell nav matrix is intact). tsc **3178** (== baseline), **0** in
  touched files. Vite build exit 0 (PWA 168 entries). **The live race was NOT browser-reproduced this slice** (no
  browser tooling; the recon DB evidence already proved the intermittency + exonerated the backend) -- the
  decouple-and-Apply fix removes the racy trigger regardless. **Manual live-cert pending Nitesh:** arrow onto the
  remarks cell -> Enter opens the editor -> type -> Enter saves + moves down a row; Esc closes back to grid nav; color a
  cell, then from a DIFFERENT cell arm a swatch + check "apply to whole row" + Apply -> the whole row fills reliably;
  rate-cell arrow/Tab/Enter nav unchanged. **NEXT = 4b** (computed-flag layer into the review-list seam) + the F1-F4
  formula builder. Full detail in boq-upload-plan.md "Slice 4a.2".
...
per-session OVERRIDE TOGGLE (default OFF). **CLOSES the Slice 3 arc.** NO migrate.

- **THE GATE KEYS ON node_type, BOTH SIDES (the no-drift rule).** The frontend gates on `row.node_type` and the server
  gates on the committed node's `node_type` -- the SAME field. Do NOT gate the frontend on `effective_classification`
  (the lowercase taxonomy axis) while the server gates on node_type; that would let the two axes drift. node_type values
  are VERBATIM "Preamble" / "Line Item" / "Other".
- **`isPriceableType(nodeType)` (NEW pure exported helper in `PricingGrid.tsx`, unit-tested).** `nodeType === "Preamble"
  || nodeType === "Line Item"` -- VERBATIM; false for "Other", null, undefined, and any lowercase/mis-cased value (no
  fuzzy match). The grid's per-row gate.
- **THE PER-ROW GATE (`PricingGrid.tsx`).** NEW `override?: boolean` prop (default false). The editable rate-cell branch
  extends from `if (onSaveRate && isRateDescriptor(d))` to `if (onSaveRate && isRateDescriptor(d) &&
  (isPriceableType(row.node_type) || override))`. When false the cell falls through to the EXISTING read-only render --
  NO new read-only branch. `row.node_type` rides the PricedRow (surfaced by the backend, see below).
- **THE AMBER "NEEDS REVIEW" MARKER (`PricingGrid.tsx`).** A priced cell on a non-priceable row (`priced &&
  !isPriceableType(row.node_type)`) is the override-priced anomaly -> rendered AMBER (bg-amber-50 + amber dot + title
  "Priced on a non-priceable row -- flagged for review") instead of the EMERALD priced marker. Composed identically in
  BOTH the editable rate-cell branch AND the read-only default fall-through -- so the anomaly stays visible when the
  override is OFF (e.g. on reload after an override save, the cell lands in the read-only branch). This is the
  lightweight 3e marker; the full review-strip flag is slice 4b (DERIVE-not-stamp -- no BoQ Cell Pricing field).
- **THE TOGGLE (`SheetPricingPage.tsx`, per-sheet per-session).** `const [override, setOverride] = useState(false)`
  (mirrors summaryOpen). A header-cluster button (beside Summary / Save now, INSIDE the `!isGridOnly` cluster so it is
  SUPPRESSED for grid-only): default OFF reads "Override" (Lock icon, outline); ON reads "Override ON" (Unlock icon,
  loud amber fill, `aria-pressed`) -- a loaded gun the user always sees is on. Resets per sheet via `setOverride(false)`
  added to the existing `useEffect([sheetName])`. Passed `override={override}` to PricingGrid. `handleSaveRate` adds
  `allow_non_priceable: override` to the saveCellPrice args (the page reads its own override state directly). A loud
  amber inline banner shows below the editor note while override is ON.
- **DERIVE-NOT-STAMP (`PricedRow.node_type`, boqTypes.ts).** `PricedRow` gains `node_type?: "Preamble" | "Line Item" |
  "Other" | null`. The anomaly ("a price on a non-priceable row") is DERIVABLE at read time from node_type + the priced
  flag -- NO marker field on BoQ Cell Pricing. 4b lights up the full flag later from the same data.
- **VERIFIED:** backend `test_pricing` **41 -> 47** (+6 `TestPriceabilityGuard`: Other rejected w/o override [+ a
  rejected write wrote no price row AND did not acquire the lock]; Other accepted w/ override; override accepts the HTTP
  string "true"; priceable saves w/ and w/o override; node_type rides the committed + priced rows). `test_review_screen`
  **205 unchanged** (the node_type emit is additive). Vitest **68 -> 72** (+4 `isPriceableType`). tsc **3178** (==
  baseline), **0** in touched files (`review_screen`-N/A frontend; filtered `SheetPricingPage|PricingGrid|boqTypes`).
  Vite build exit 0. NO migrate. Live API cert: get_priced_rows delivers node_type per row (Preamble/Other/Line Item);
  _resolve_committed_cell returns `{name, node_type:'Other'}`. **Manual live-cert pending Nitesh (the toggle + marker
  are JSX = manual-cert):** on a data sheet with notes/spacers, a non-priceable row's rate cell is read-only by default;
  flip Override ON (loud amber button + amber banner) -> the non-priceable rate cell becomes editable; save a rate on it
  -> AMBER "needs review" marker, priceable cells stay emerald; turn Override OFF (or switch sheets and back) -> the
  override resets, the amber-priced cell stays amber + read-only; a direct non-priceable save without the override is
  rejected server-side. Full detail in boq-upload-plan.md "Slice 3e".
...
priced/un-priced marker -- reusing the Slice-2 `reviewRender` helpers in a NEW grid component (design v1.3 Sec.4 path b
-- it does NOT import/reuse/retune the ReviewTree component). NO editing (3b), NO Save/Export/Finalize (3c/5), NO
backend, NO migrate.

- **Route (`routesConfig.tsx`, additive).** `upload-boq/hub/:boqId/pricing/:sheetName` -> lazy
  `@/pages/boq-wizard/SheetPricingPage` (exports `{ SheetPricingPage as Component }`). 5th wizard sibling; existing 4
  routes unchanged. sheetName is `encodeURIComponent`'d on nav, RR v6 auto-decodes -> verbatim sheet_name.
- **`SheetPricingPage.tsx` (NEW) -- shell mirrors SheetReviewPage.** `useParams(boqId, sheetName)`;
  `useFrappeGetDoc<BOQsDoc>("BOQs", ...)` for the header; `useFrappeGetCall<{ message: GetPricedRowsResponse }>(
  "...pricing.get_priced_rows", { boq_name, sheet_name }, boqId && sheetName ? undefined : null)`. Full-page spinner
  while BOQs loads; inline loading (`data===undefined`) / error (`data===null`) for the grid; Back nav to the hub
  (`/upload-boq/hub/:boqId`, entity-id convention -- never navigate(-1)). **Header = Back + title ONLY**
  (`{boq_name} · V{version} · Pricing · committed v{commit_version}` + the sheet name); NO Save/Export buttons (nothing
  to save read-only). A muted read-only note strip sets the expectation. **`editable` / `lock_info`** are read from the
  payload and passed to PricingGrid as INERT reserved props (the future single-editor-lock hook for 3b; no lock logic).
- **`PricingGrid.tsx` (NEW) -- the read-only grid.** Imports `computeDepths` / `resolveDescriptorValue` /
  `renderDescriptorCell` / `ClassificationPill` from `./reviewRender` + `ROLE_LABELS` from `./boqTypes`. **Does NOT
  import ReviewTree** (the locked path-b call) -- `FIXED_ROLE_DEDUPE` is therefore defined LOCALLY (a 2-role
  `Set(["sl_no","description"])`, documented as a kept-in-sync mirror of ReviewTree's) and `INDENT_PX=20` is mirrored
  locally. Mirrors ReviewTree's descriptor-render loop MINIMALLY: fixed anchors **Excel Row / Sl.No / Parent**
  (parent's `source_row_number`, read-only -- muted "↑ N", NO scroll-to nav) **/ Classification** (`<ClassificationPill
  cls={row.effective_classification} />`, NO chevron) **/ Description** (depth indent `paddingLeft = depth * INDENT_PX`
  from `computeDepths`, with preamble/line_item styling), then `displayDescriptors = columnDescriptors.filter(d =>
  !FIXED_ROLE_DEDUPE.has(d.role))` with header `${col} — ${ROLE_LABELS[role] ?? role}${d.area ? ' · '+area : ''}` and
  body `resolveDescriptorValue(row, d)` -> `renderDescriptorCell(val)`. **Omitted (minimal read-only):** detail panel,
  inline edit, reclassify, AI columns, restructure modal, remarks, search/filter, column-subset selector, collapse,
  and the per-area header tint (area is shown in the label; `buildAreaColorMap` is ReviewTree-local/not exported, so
  skipped to stay decoupled).
- **PRICED/UN-PRICED MARKER (Q4 -- basic visual, IN for 3a).** Each RATE cell that carries a saved price renders a
  subtle emerald tint + a small dot, driven SOLELY by the overlay's `priced_*` markers -- per-area
  `priced_by_area[area][kind] === true` or scalar `priced_rate_<kind> === true` -- **NEVER a zero-check** (a committed
  0.0 rate can be a valid priced value). Two PURE exported helpers do the derivation: `isRateDescriptor(d)` (rate cell
  iff `value_field === "rate_by_area"` or in `{rate_supply,rate_install,rate_combined}`) + `isCellPriced(row, d)`. Only
  RATE cells get a marker (amount/qty never). The rich review-flag layer ("needs a rate" / "won't compute") is Slice 4b.
...
- **Verification (observed, in-container).** Vitest **20/20 GREEN** (12 Slice-2 reviewRender + 8 NEW PricingGrid marker
  ...
  **0** in the touched wizard files (filtered `boq-wizard|SheetPricingPage|PricingGrid|BoqHubPage|SheetCard|boqTypes|
  routesConfig`). Vite build exit 0 (PWA **166** entries -- +2 lazy chunks for the page + grid). **Manual live-cert
  pending Nitesh:** committed SheetCards show a Price button (non-committed don't); Price -> the pricing page loads +
  renders rows/pills/depth/cells visually like the review screen; priced rate cells show the marker, un-priced don't
  (0.0-priced reads priced); Back returns to the hub. **Slice 3a unblocks 3b (inline rate editing).** Full detail in
