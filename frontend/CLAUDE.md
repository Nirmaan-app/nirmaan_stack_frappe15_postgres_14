...
**Status (2026-06-02):** Module 1b COMPLETE. Module 2b-i ✅ COMPLETE (feat 81568df9;
...
**Section-grain confirmedFields keys (Slice 3e feat e60e768c; 3e-fix):** SheetConfigPanel's `confirmedFields: Set<string>` includes three stable section-level keys alongside per-field keys: `"section:rows"`, `"section:areas"`, `"section:roles"`. A section is confirmed when: (a) any field in the section is interacted with (focus or change events both call `touchS1/touchS2/touchS3` which set both the field key AND the section key), (b) the user clicks "Accept all sections as-is" (adds the full `SAVE_ALL_FIELDS` set -- all 6 per-field keys + 3 section keys -- matching Save semantics so every sparkle clears), or (c) a Save or Mark-as-reviewed completes (all keys added via `SAVE_ALL_FIELDS`). Section h3 headings show the sparkle when their section key is unconfirmed. Section 2 and Section 3 headings also OR-check their per-field key (area_dimensions / column_role_map) -- bulk-accept must therefore add the full SAVE_ALL_FIELDS set (not just the 3 section keys) to clear all sparkles. Layer 1 is satisfied when all three section keys are present.

**Save-then-status two-call Mark-reviewed pattern (Slice 3e feat e60e768c):** A save-anchored "mark as reviewed" action always calls `set_sheet_config` first and `set_sheet_status("Reviewed")` second. Never skip the config save step (the user attests to what's persisted). If the config save fails, do NOT call set_sheet_status. If the status flip fails after a successful config save, report it inline -- the config IS saved; only the status flip failed and the user can retry. Both calls use the same `isSaving` flag; errors surface via `saveError` inline (no toasts).
