...
export const ProjectPRSummaryTable: React.FC<ProjectPRSummaryTableProps> = ({
  ...
  const getPREstimatedTotal = useMemo(
    () =>
      memoize(
        (
          pr: ProcurementRequest | undefined,
          derivedStatus: string,
          projectPOs: ProcurementOrder[] | undefined
        ): number | "N/A" => {
          if (!pr) return "N/A";
          let total = 0;

          if (derivedStatus === "Approved PO") {
            const filteredPOsForThisPR =
              projectPOs?.filter((po) => po.procurement_request === pr.name) ||
              [];
            ...
            // New PR or Open PR - use estimated quotes
            // console.log("DEBUG: pr.order_list", pr);
            pr.order_list?.forEach((item) => {
              if (item.status !== "Deleted") {
                // Only consider non-deleted items for estimation
                const minQuoteInfo = getThreeMonthsLowestFiltered(
                  quote_data,
                  item.name
                ); // Use item_code or item
                total +=
                  parseNumber(minQuoteInfo?.averageRate || 0) *
        ...
        (
          ...
          (pr?.name || "none") +
          derivedStatus +
          (projectPOs ? projectPOs.map((p) => p.name).join(",") : "")
      ),
  ...
  const columns = useMemo<ColumnDef<ProcurementRequest>[]>(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Creation On" />
        ),
        cell: ({ row }) => (
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="font-medium pr-2">
              {estimateTotal === "N/A"
                ? "N/A"
                : formatToRoundedIndianRupee(estimateTotal)}
            </div>
          );
        },
        size: 150,
        enableSorting: false, // Sorting on calculated values is client-side
        meta: {
