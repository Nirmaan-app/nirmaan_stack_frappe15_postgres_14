...
export const ProjectPOSummaryTable: React.FC<ProjectPOSummaryTableProps> = ({ projectId }) => {
    ...
    const vendorOptions = useMemo(() => vendors?.map((ven) => ({ label: ven.vendor_name, value: ven.name })) || [], [vendors]);

    // --- Memoized Lookups ---
    const getVendorName = useCallback(memoize((vendorId?: string) => vendors?.find(v => v.name === vendorId)?.vendor_name || vendorId || "--"), [vendors]);

    const getWorkPackageName = useMemo(() => memoize((po: ProcurementOrder): string => {
        if (po.custom === "true") return "Custom"; // If PO itself is custom
        const relatedPR = pr_data?.find(pr => pr.name === po.procurement_request);
    ...
    const getTotalAmountPaidForPO = useMemo(() => {
        ...
        projectPayments.forEach(p => { // Assuming projectPayments is already filtered for "Paid" PO payments
            if (p.document_name) {
    ...
    const staticFilters = useMemo(() => {
        const filters: Array<[string, string, any]> = [["status", "not in", ["Cancelled", "Merged"]]];
        if (projectId) {
    ...
    const urlSyncKey = useMemo(() => `prj_po_summary_${projectId || 'all'}`, [projectId]);
    ...
    if (combinedErrorOverall && !poDataForPage?.length && !poAggregates) {
        toast({ title: "Error Loading PO Summary", description: combinedErrorOverall.message, variant: "destructive" });
    }
