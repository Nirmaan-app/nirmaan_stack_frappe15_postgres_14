...
export const ReviewPlansPage: React.FC<ReviewPlansPageProps> = ({
    ...
    // Update delivery date for a plan
    const updateDeliveryDate = (planIndex: number, date: Date | undefined) => {
        const updatedPlans = [...plans];
        updatedPlans[planIndex].deliveryDate = date
    ...
    const isValid = useMemo(() => {
        return plans.every(
            (plan) => plan.selectedItems.size > 0 && plan.deliveryDate
        );
    }, [plans]);
            ...
            <div className="flex items-center justify-between">
                ...
                {/* {(categoryName || taskName) && (
                    <div className="text-sm text-right">
                        ...
                        {taskName && (
                            <div>Task: <span className="font-medium">{taskName}</span></div>
                        )}
                    </div>
                )} */}
            </div>
...
export default ReviewPlansPage;
