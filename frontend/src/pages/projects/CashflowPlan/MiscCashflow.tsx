...
export const MiscCashflow = () => {
    const { projectId } = useParams<{ projectId: string }>();
    if (!projectId) return <div className="text-red-500">Project ID missing</div>;
    return <MiscCashflowContent projectId={projectId} />;
};

const MiscCashflowContent = ({ projectId }: { projectId: string }) => {
                ...
                <div className="flex justify-between items-center mb-6">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                             <h3 className="text-xl font-bold text-gray-900">Miscellaneous Plan</h3>
                             {plans && (
                        ...
                        <p className="text-sm text-gray-500">Create and manage ad-hoc expenses that don't fall under standard work orders or material plans.</p>
                    </div>
                    {/* Add Button */}
                    {!showAddForm && (
                        <Button 
                            ...
                            <div key={plan.name} className="border rounded-lg bg-white overflow-hidden transition-all hover:shadow-sm">
                                <div className="flex items-center p-4 gap-4">
                                    ...
                                    <div className="w-[180px] shrink-0">
                                        <Badge variant="secondary" className="mb-1 text-[10px] text-purple-700 bg-purple-50 px-1.5 rounded-sm uppercase tracking-wider">
                                            Plan {index + 1}
                                        </Badge>
