...
/**
 * May be SENT to the BoQ wizard hub -- i.e. would the router actually let them in.
 *
 * ⚠️ DERIVED FROM `UPLOAD_BOQ_ACCESS`, NOT A FOURTH HAND-LISTED SET. That constant IS the
 * route guard (`<RoleRoute allowed={UPLOAD_BOQ_ACCESS}>` on `/upload-boq/*`), so reading it
 * here makes drift between "we navigate them there" and "the router admits them" impossible
 * to express. Do not re-spell the membership as a local Set to save the import.
 *
 * WHY THIS EXISTS SEPARATELY FROM `canOpenBoqWizard`. That one answers "does this user get a
 * PENCIL", and is the narrower authoring set. This one answers "is a hub URL a place we may
 * send them at all", and is the wider route set -- procurement and Project Lead are in this
 * one and not that one. The pre-Won row click uses THIS: viewing a pre-Won stub is not
 * limited to the four wizard profiles, so an Accountant or Sales user can reach the BoQ list
 * on a stub, and navigating them to the hub would land them on Access Denied.
 */
export function canOpenBoqHub(role: string | undefined, userId: string | undefined): boolean {
  if (userId === "Administrator") return true;
  return !!role && UPLOAD_BOQ_ACCESS.includes(role);
}
