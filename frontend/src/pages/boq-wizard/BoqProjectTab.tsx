...
const BoqProjectTab = ({ projectId }: BoqProjectTabProps) => {
  ...
  // page does not move.
  //
  // ⚠️ KNOWN CONSEQUENCE, accepted by the owner: this row click was the ONLY route into an
  // EXISTING BoQ's hub for the roles that hold route access but get no pencil (procurement,
  // Project Lead) -- BoqUploadScreen's Continue only opens a BoQ uploaded in that same session.
  // They now have no way to reach the hub from the UI. Widening canOpenBoqWizard is the fix if
  // that is ever reported as a bug; do not "restore" this redirect to paper over it.
