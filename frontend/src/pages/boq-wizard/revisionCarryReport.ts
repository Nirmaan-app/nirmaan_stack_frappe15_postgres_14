...
/** The commit envelope's optional per-sheet `revision_overlay`. `provenance` is a 1/0 flag. */
export interface RevisionOverlaySummary {
  provenance: number;
  formulas: number;
  remarks: number;
  colors: number;
  remark_dismissals: number;
  categories: number;
}

/**
 * The carried layers, in the order the backend carries them. `provenance` is deliberately absent:
 * it is a 1/0 "this is a revision sheet" flag, not a row count, so listing it would read as
 * "carried 1 provenance".
 */
const OVERLAY_LAYERS: ReadonlyArray<
  [keyof RevisionOverlaySummary, string, string]
> = [
  ["formulas", "amount formula", "amount formulas"],
  ["remarks", "remark", "remarks"],
  ["colors", "color", "colors"],
  ["remark_dismissals", "flag dismissal", "flag dismissals"],
  ["categories", "category", "categories"],
];

/**
 * One committed sheet's overlay line for the commit-results modal.
 *
 * Returns null off a revision sheet (the key is absent, or provenance is 0). A revision sheet that
 * carried no annotations still gets a line -- silence there is indistinguishable from "not a
 * revision", and the whole point of W5 is that the user can tell.
 */
export function formatRevisionOverlay(
  overlay: RevisionOverlaySummary | undefined | null,
): string | null {
  if (!overlay || !overlay.provenance) return null;

  const parts = OVERLAY_LAYERS
    .filter(([key]) => (overlay[key] ?? 0) > 0)
    .map(([key, one, many]) => plural(overlay[key], one, many));

  if (parts.length === 0) return "revision of the original — no annotations to carry";
