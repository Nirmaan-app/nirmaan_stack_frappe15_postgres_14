...
export const ApproveSelectAmendPO: React.FC = () => {
    ...
    // --- Fields to Fetch ---
    const fieldsToFetch = useMemo(() => DEFAULT_PO_FIELDS_TO_FETCH.concat(['creation', 'modified', 'order_list', 'loading_charges', 'freight_charges', 'procurement_request']), []);

    // --- Date Filter Columns ---
    const dateColumns = PO_DATE_COLUMNS

    const poSearchableFieldsOptions = useMemo(() => PO_SEARCHABLE_FIELDS.concat([{ value: "procurement_request", label: "PR ID", placeholder: "Search by PR ID..." }, { value: "owner", label: "Created By", placeholder: "Search by Created By..." }]), []);
