...
export const InFlowPayments: React.FC<InFlowPaymentsProps> = ({
  ...
  const projectsFetchOptions = getProjectListOptions({
    filters: projectFiltersForLookup as Filter<FrappeDoc<Projects>>[],
  });
  ...
  const customersFetchOptions = getCustomerListOptions({
    filters: customerFilters as Filter<FrappeDoc<Customers>>[],
  });
  ...
  const getCustomerName = useCallback(
    memoize(
      (custId?: string) =>
        customers?.find((c) => c.name === custId)?.company_name ||
        custId ||
        "--"
    ),
    [customers]
  ...
  const confirmDeleteItem = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to delete inflow.",
        variant: "destructive",
      });
    } finally {
  ...
  const columns = useMemo<ColumnDef<ProjectInflows>[]>(
    () => [
      ...
      },
      {
        accessorKey: "utr",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="Payment Ref (UTR)" />
        ),
        cell: ({ row }) => {
          // console.log("row", row.original)
          const data = row.original;
          return data.inflow_attachment ? (
            <a
              href={SITEURL + data.inflow_attachment}
              target="_blank"
              rel="noreferrer"
      ...
      ...(!customerId
        ? [
            {
              // Conditionally show Customer column
        ...
        : []),
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium text-green-600 pr-2">
                  ...
                  <div>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() => handleOpenEditDialog(inflow)}
                        >
                        ...
                        {canDelete && (
                          <>
                            ...
                            <DropdownMenuItem
                              onClick={() =>
                                handleOpenDeleteConfirmation(inflow)
                              }
  ...
  if (
    combinedErrorOverall &&
    !data?.length &&
    !isLoadingOverall &&
    !listIsLoading
  ) {
    // Check loading states too
        ...
        <DataTable<ProjectInflows>
          ...
          summaryCard={
            <InflowSummaryCard
              ...
              customerName={
                customerId ? getCustomerName(customerId) : undefined
              }
            />
          }
        />
      ...
      {inflowToDelete && (
        ...
        >
          <AlertDialogContent>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={() => setInflowToDelete(null)}>
                Cancel
              </AlertDialogCancel>
              <AlertDialogAction
