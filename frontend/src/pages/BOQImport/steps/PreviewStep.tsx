...
import { CheckCircle2, Loader2, AlertTriangle, FileSpreadsheet, Info, BookOpen } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { formatCurrency, formatNumber } from '../constants';
import type { BOQPreviewRow, BOQImportResult, BOQUploadFormValues, FieldMapping, PreambleEntry, RowClassification,} from '../schema';
...
export function PreviewStep({
  ...
  const [classifications, setClassifications] = useState<RowClassification[]>([]);
  const [preambles, setPreambles] = useState<PreambleEntry[]>([]);
  const [isClassifying, setIsClassifying] = useState(false);

  const { call: classifyCall } = useFrappePostCall(
    'nirmaan_stack.api.boq_import.parse_excel_preview'
  );

  // Fetch classification from backend when component mounts or mapping changes
  useEffect(() => {
    if (!fileUrl || Object.keys(fieldMapping).length === 0) return;
    ...
    classifyCall({
      ...
      .then((res: any) => {
        ...
        setClassifications(data.row_classifications || []);
        setPreambles(data.preambles || []);
      })
      .catch((err: any) => {
        if (!cancelled) {
  ...
  }, [fileUrl, fieldMapping, dataStartRow, selectedSheetIndex]);

  // Build a set of preamble row indices for quick lookup
  const preambleRowSet = useMemo(() => {
    const set = new Set<number>();
    for (const c of classifications) {
      if (c.type === 'preamble') set.add(c.row_index);
    }
    return set;
  }, [classifications]);

  // Build a map of row_index → preamble_id from classifications
  const rowToPreambleId = useMemo(() => {
    const map = new Map<number, number | null>();
    let currentPreambleId: number | null = null;
    for (const c of classifications) {
      if (c.type === 'preamble') {
        currentPreambleId = c.preamble_id ?? null;
      } else if (c.type === 'item') {
        map.set(c.row_index, currentPreambleId);
      }
    }
    return map;
  }, [classifications]);

  // Build preamble lookup map
  ...
  // Transform preview rows into typed BOQ rows, excluding preambles
  const mappedRows = useMemo(() => {
    ...
    const result: BOQPreviewRow[] = [];

    for (let idx = 0; idx < previewRows.length; idx++) {
      // Skip preamble rows (backend classified)
      if (preambleRowSet.has(idx)) continue;
      ...
      let hasWarning: BOQPreviewRow['hasWarning'];
      if (supply_rate === 0 && installation_rate === 0) hasWarning = 'zero_rates';
      ...
      result.push({
        ...
        amount,
        hasWarning,
        preamble_id: rowToPreambleId.get(idx) ?? null,
      });
    }

    return result;
  }, [previewRows, colIndexMap, preambleRowSet, rowToPreambleId]);
  ...
  const handleImport = async () => {
    try {
      ...
      setImportResult({
        ...
        preambles_count: data.preambles_count,
      });

      toast({
        title: 'BOQ Imported Successfully',
        description: `${data.items_count} items imported (${data.preambles_count || 0} preamble sections detected)`,
      });
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          {(importResult.preambles_count ?? 0) > 0 && (
            <p className="text-xs text-muted-foreground">
              {importResult.preambles_count} preamble sections extracted
            </p>
          )}
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-4">
          ...
          <CardContent className="p-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-semibold tabular-nums">
                {isClassifying ? '...' : summary.totalItems}
              </p>
            </div>
          </CardContent>
        </Card>
        <Card className="flex-1 min-w-[140px] shadow-sm">
          ...
          {preambles.length > 0 && (
            <Card className="flex-1 min-w-[140px] shadow-sm border-blue-200">
              <CardContent className="p-3 flex items-center gap-3">
                <BookOpen className="h-5 w-5 text-blue-500" />
                <div>
                  <p className="text-xs text-muted-foreground">Preambles</p>
                  <p className="text-lg font-semibold tabular-nums">{preambles.length}</p>
                </div>
              </CardContent>
            </Card>
          )}
        ...
        <p className="text-xs text-muted-foreground">
          Showing preview of {mappedRows.length} items (of {totalRows} total rows,{' '}
          {preambles.length} preamble sections excluded)
        </p>
      ...
      >
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {mappedRows.map(row => {
                const preambleText =
                  row.preamble_id != null ? preambleMap.get(row.preamble_id) : undefined;
                return (
              <tr
                key={row.rowIndex}
                className={cn(
                    ...
                    <span className="line-clamp-2">{row.description}</span>
                        {preambleText && (
                          <Tooltip>
                            ...
                            >
                              <p className="font-semibold text-blue-600 mb-1">Preamble</p>
                              <p>{preambleText}</p>
                            </TooltipContent>
                          </Tooltip>
                        )}
      ...
      <FormActions align="between">
        ...
        <Button onClick={handleImport} disabled={isImporting || mappedRows.length === 0}>
          {isImporting ? (
            ...
            `Import ${summary.totalItems} Items`
          )}
        </Button>
      </FormActions>
    </div>
