...
export const usePOReportsData = (): UsePOReportsDataResult => {
    ...
    const reportData = useMemo<POReportRowData[] | null>(() => {
        ...
        if (poLoading || projectsLoading || vendorsLoading) {
            return null;
        }
        ...
        (purchaseOrders || []).forEach(po => {
            if (po) {
                ...
                combinedData.push({
                    ...
                    invoiceAmount: parseNumber(po.amount_invoiced),
        ...
        purchaseOrders, projects, vendors,
        poLoading, projectsLoading, vendorsLoading,
        projectMap, vendorMap,
    ]);

    // --- Consolidated Loading and Error State ---
    const isLoading = poLoading || projectsLoading || vendorsLoading || assigneesLoading;
    const error = poError || projectsError || vendorsError || assigneesError;
