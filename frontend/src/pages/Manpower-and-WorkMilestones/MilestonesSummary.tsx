...
if (projectData?.project_work_packages) {
                  ...
                  <CardContent className="flex flex-col gap-4">
                    ...
                    <div className="mt-4">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                        {/* Placeholder for images - if you have attachments, you'd map them here */}
                        <div className="w-full h-24 bg-gray-200 flex items-center justify-center text-xs text-gray-500 rounded">
                          No Photos
                        </div>
                        {/* Example if you had image URLs in todayReportDetails.attachments:
                        {todayReportDetails.attachments?.map((attachment, idx) => (
                          <img key={idx} src={attachment.file_url} alt="Project Photo" className="w-full h-24 object-cover rounded"/>
                        ))}
                        */}
                      </div>
                    </div>
 ...
 // useEffect(() => {
  ...
  //     const reportForToday = projectProgressReports.some(
  //       (report) => formatDate(report.report_date) === today
  //     );
  //     setTodayReportAvailable(reportForToday);
  //   }
  // }, [projectProgressReports, selectedProject]);
