 ...
 * a facet fetched from `Project Payments` would offer no `Source` values at all,
...
import { APPROVAL_STATUS, SOURCE_LABEL } from "./approvalsTable.config";
...
/** Closed sets — no round trip earns its keep for three values. */
const SOURCE_OPTIONS = (Object.keys(SOURCE_LABEL) as (keyof typeof SOURCE_LABEL)[]).map(
  (k) => ({ label: SOURCE_LABEL[k], value: k }),
);
...
export function useApprovalFacets({
  ...
  return useMemo(() => {
    const map: FacetOptionMap = {
      source: { title: "Source", options: SOURCE_OPTIONS },
      ...
      // simply not reachable through these two facets. Deliberate: the Source
