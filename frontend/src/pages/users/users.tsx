...
export const USER_LIST_FIELDS_TO_FETCH: (
  | keyof NirmaanUsers
  | "name"
  | "email"
  | "mobile_no"
...
const UsersSummaryCard: React.FC = () => {
  ...
  // Fetch total users
  const { data: totalCountData, isLoading: totalCountLoading } =
  ...
  const columns = useMemo<ColumnDef<NirmaanUsers>[]>(
    () => [
      ...
      },
      {
        ...
        meta: {
          ...
          exportValue: (row: NirmaanUsers) => {
            ...
            const roleLabel =
              USER_ROLE_PROFILE_OPTIONS.find((opt) => opt.value === roleValue)
                ?.label ||
              roleValue?.replace("Nirmaan ", "").replace(" Profile", "");
            return roleLabel;
          },
        },
      },
      ...
      // },
      {
        ...
        cell: ({ row }) => (
          <div className="font-medium">{row.getValue("mobile_no") || "--"}</div>
        ),
        size: 150,
        meta: {
