...
export const ApproveSelectSentBack: React.FC = () => {
  ...
  const getTotal = useMemo(
    () =>
      memoize((order_list: ProcurementRequestItemDetail[]): number => {
        let total = 0;
  ...
  const handleNewSBSeen = useCallback(
  ...
  const fieldsToFetch = useMemo(
    () =>
      DEFAULT_SB_FIELDS_TO_FETCH.concat([
        "creation",
        "modified",
        "item_list",
        "procurement_request",
  ...
  const sbSearchableFields = useMemo(
    () =>
      SB_SEARCHABLE_FIELDS.concat([
        {
  ...
  const columns = useMemo<ColumnDef<SentBackCategory>[]>(
    () => [
      {
        accessorKey: "name",
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title="SB ID" />
        ),
        cell: ({ row }) => {
          ...
          const isNew = notifications.find(
            (item) =>
              item.docname === sbId &&
              item.seen === "false" &&
              item.event_id === "sb:vendorSelected" // Ensure correct event_id
          );
          return (
            <div
              role="button"
              tabIndex={0}
              onClick={() => handleNewSBSeen(isNew)}
            ...
            >
              ...
              <Link
                ...
                to={`/purchase-orders/${sbId}?tab=Approve Sent Back PO`}
              >
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <p className="font-medium pr-2">
            {formatToRoundedIndianRupee(getTotal(row.original.order_list))}
          </p>
        ),
        size: 150,
        enableSorting: false,
    ...
    [notifications, projectOptions, userList, handleNewSBSeen, getTotal]
