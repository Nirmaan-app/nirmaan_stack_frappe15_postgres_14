...
export const SBQuotesSelectionReview: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Persist the loss justifications onto the order_list child rows alongside
      // the workflow transition (no dedicated SB send endpoint exists).
      const updatedOrderList = (orderData?.order_list || []).map((it) => ({
        ...it,
        loss_justification:
          justifications[it.name] !== undefined
            ? justifications[it.name]
            : it.loss_justification,
      }));
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button
                  ...
                  disabled={!allVendorsHaveTerms || !allHighLossJustified || update_loading}
                >
