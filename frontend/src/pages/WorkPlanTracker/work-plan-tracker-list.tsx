...
import { WorkPlanProjectCard } from "./components/WorkPlanProjectCard";
...
const API_PATH = "nirmaan_stack.api.seven_days_planning.get_projects_with_stats.get_projects_with_work_plan_stats";
const ADMIN_ROLE = "Nirmaan Admin Profile";

const WorkPlanTrackerList: React.FC = () => {
  ...
  // Fetch projects with Work Plan stats
  const {
    data: apiResponse,
    isLoading,
    error,
    mutate: refetchList,
  } = useFrappeGetCall<{ message: ProjectWithWorkPlanStats[] }>(
    API_PATH,
    {},
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      // Status filter: empty selection => show all projects
      if (
  ...
  const handleProjectClick = (projectId: string) => {
    navigate(`/work-plan-tracker/${projectId}`);
  };
  ...
  return (
    <div className="flex-1 p-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Track work plan activities across all projects
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
        <div className="relative flex-1 min-w-0">
          ...
          <Input
            ...
            className="pl-10 h-10 text-sm border-gray-300 focus:ring-2 focus:ring-primary/20"
          />
        </div>
      ...
      {/* Project Cards Grid */}
      {projects.length === 0 ? (
        // Empty state - no projects have work plans
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-gray-500 max-w-md">
            No projects have work plan activities yet. Enable milestone tracking
            and create work plans from within a project&apos;s Planning tab.
          </p>
        </div>
...
export default WorkPlanTrackerList;
