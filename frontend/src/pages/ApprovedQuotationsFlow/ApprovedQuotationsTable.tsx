...
export default function ApprovedQuotationsTable({
  ...
  // Deduplicate items based on item_name (since we are fetching from Approved Quotations which has duplicates)
  ...
  // Map item_name -> all document names (IDs) for that item_name.
  // Used to convert user's item_name selection into safe document "name" filters.
  const itemNameToNamesMap = useMemo(() => {
    ...
    for (const item of allItems) {
      ...
      if (existing) {
        ...
        map.set(item.item_name, [item.name]);
      }
    }
    return map;
  }, [allItems]);
  ...
  useEffect(() => {
    ...
    const allOptions = uniqueAllItems || [];
    ...
    if (!trimmedInput) {
       ...
       setFilteredSuggestions(allOptions.filter(item => !selectedItemNames.has(item.item_name)).slice(0, 100)); // slice to prevent rendering too many
       return;
    }
  ...
  const handleItemSelect = useCallback(
    (item: ItemsType) => {
      ...
      if (!selectedItems.some((selected) => selected.value === item.item_name)) {
        setSelectedItems((prev) => [
          ...prev,
          { value: item.item_name, label: item.item_name },
        ]);
      }
