...
export const PaymentTermsDialog: React.FC<PaymentTermsDialogProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        ...
        setStep("details");
        const sanitizedTerms = initialData.terms?.map(term => ({ ...term, amount: roundToTwo(term.amount) })) || [];
        ...
        setStep("select"); setType(null); setMilestones([]);
      }
    }
  }, [isOpen, initialData]);

  // --- Derived Data & Validation (Using your more robust logic) ---
  const totalAmount = useMemo(() => roundToTwo(milestones.reduce((sum, m) => sum + (m.enabled ? m.amount : 0), 0)), [milestones]);
  const totalPercentage = useMemo(() => milestones.reduce((sum, m) => sum + (m.enabled ? m.percentage : 0), 0), [milestones]);
  const pendingAmount = useMemo(() => roundToTwo(poAmount - totalAmount), [poAmount, totalAmount]);
  const isTotalAmountValid = useMemo(() => pendingAmount < 1 && pendingAmount >= 0, [pendingAmount]);
  const areCreditDatesValid = useMemo(() => {
    if (type !== 'Credit') return true;
    return milestones.every(m => m.enabled && m.due_date);
  }, [type, milestones]);
  const isFormValid = useMemo(() => isTotalAmountValid && areCreditDatesValid, [isTotalAmountValid, areCreditDatesValid]);
  ...
  const handleTypeSelectionConfirm = () => {
    if (type) {
      if (type === "Credit") {
        setMilestones([{ id: `term_${Date.now()}`, name: "1st Payment", amount: 0, percentage: 0, due_date: "", enabled: true }]);
      } else if (type === "Delivery against payment") {
        setMilestones(DELIVERY_TERM_NAMES.map((name) => ({ id: `term_${Date.now()}_${name.replace(/\s+/g, "")}`, name, amount: 0, percentage: 0, enabled: false })));
      }
      setStep("details");
    }
  };
  ...
  const handleAddMilestone = () => {
    if (type === "Credit") {
      ...
      setMilestones([...milestones, { id: `term_${Date.now()}`, name: `${getOrdinal(newPaymentNumber)} Payment`, amount: 0, percentage: 0, due_date: "", enabled: true }]);
    }
  };

  const handleRemoveMilestone = (id: string) => { setMilestones(milestones.filter((m) => m.id !== id)); };
  const handleEnableTerm = (id: string, checked: boolean) => { setMilestones(milestones.map((m) => m.id === id ? { ...m, enabled: checked, percentage: checked ? m.percentage : 0, amount: checked ? m.amount : 0 } : m)); };
  ...
  const handleMilestoneChange = (id: string, field: keyof PaymentTermMilestone, value: string | number) => {
    ...
    if (type === 'Credit' && field === 'amount') {
        const otherMilestonesTotal = roundToTwo(milestones.reduce((sum, ms) => (ms.id !== id ? sum + ms.amount : sum), 0));
        const maxAllowedForThisField = roundToTwo(poAmount - otherMilestonesTotal);
        let newAmount = parseNumber(value);
        if (newAmount > maxAllowedForThisField) { newAmount = maxAllowedForThisField; }
        currentMilestone.amount = roundToTwo(newAmount);
        currentMilestone.percentage = poAmount > 0 ? (currentMilestone.amount / poAmount) * 100 : 0;
    ...
    // --- Logic for 'Delivery against payment' type (integrating my strict capping) ---
        ...
        let maxPercent = 100;
        if (index === 1) { // Material Readiness
            const advancePercent = newMilestones[0].enabled ? (newMilestones[0].percentage || 0) : 0;
            maxPercent = 100 - advancePercent;
        }
        // Enforce the maximum value typed by the user
        if (newPercentage > maxPercent) { newPercentage = maxPercent; }
  ...
  const handleFocus = (e: React.FocusEvent<HTMLInputElement>) => { if (e.target.value === '0') e.target.value = ''; };

  const handleFinalConfirm = () => {
    if (type && isFormValid) {
      onConfirm({ type, total_po_amount: poAmount, terms: milestones.filter((m) => m.enabled) });
      onClose();
    }
  };
  ...
  const renderSelectStep = () => (
    <>
      ...
      <div className="grid items-center justify-center gap-4 py-8 sm:flex">
        <Label htmlFor="payment-type" className="sm:text-right">Type* :</Label>
        <Select value={type ?? undefined} onValueChange={(v: VendorPaymentTerm["type"]) => setType(v)}>
          <SelectTrigger id="payment-type" className="w-full sm:w-[400px]"><SelectValue placeholder="Select Type" /></SelectTrigger>
          <SelectContent><SelectItem value="Credit">Credit</SelectItem><SelectItem value="Delivery against payment">Delivery against payment</SelectItem></SelectContent>
        </Select>
      </div>
      <DialogFooter className="flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-0">
        <Button variant="outline" onClick={onClose} className="w-full sm:w-auto">Cancel</Button>
        <Button onClick={handleTypeSelectionConfirm} disabled={!type} className="w-full sm:w-auto">Confirm</Button>
      </DialogFooter>
    </>
  );
  ...
  const renderDetailsStep = () => (
    <>
      ...
      <div className="space-y-4 py-4">
        <div><Label htmlFor="payment-type-disabled">Payment Type* :</Label><Input id="payment-type-disabled" value={type || ""} disabled /></div>
        ...
        {type === "Credit" && (<Button variant="outline" size="sm" onClick={handleAddMilestone} className="w-full"><PlusCircle className="mr-2 h-4 w-4" /> Add more</Button>)}
        <div className="space-y-1">
          <div className="flex justify-between items-center font-medium"><Label>Pending Amount :</Label><span className="text-red-500">{isOverBudget ? "Rs.0.00" : formatToIndianRupee(pendingAmount)}</span></div>
          {!isTotalAmountValid && (<div className="flex items-center gap-2 text-xs text-red-500"><span className="h-4 w-4 flex items-center justify-center rounded-full border border-red-500">!</span><span>{isOverBudget ? `Total cannot exceed 100%. Current: ${(totalPercentage || 0).toFixed(2)}%` : "The total amount must exactly match the PO Amount to confirm."}</span></div>)}
          {!areCreditDatesValid && type === 'Credit' && (<div className="flex items-center gap-2 text-xs text-red-500"><span className="h-4 w-4 flex items-center justify-center rounded-full border border-red-500">!</span><span>A due date is required for every 'Credit' payment term.</span></div>)}
        </div>
      </div>
      <DialogFooter>
        <Button variant="outline" onClick={onClose}>Cancel</Button>
        <Button onClick={handleFinalConfirm} disabled={!isFormValid}>Confirm</Button>
      </DialogFooter>
    </>
  );
  ...
  const renderCreditTable = () => (
    <table className="w-full text-sm">
      <thead className="bg-gray-100"><tr><th className="p-2 text-left font-medium">TERM</th><th className="p-2 text-left font-medium">AMOUNT</th><th className="p-2 text-left font-medium">DUE DATE</th><th className="p-2 text-left font-medium">PERCENTAGE</th><th className="p-2"></th></tr></thead>
      <tbody>
        {milestones.map((m, index) => {
          const otherMilestonesAmount = roundToTwo(totalAmount - (m.enabled ? m.amount : 0));
          const maxAllowed = roundToTwo(poAmount - otherMilestonesAmount);
          return (<tr key={m.id} className="border-t">
              <td className="p-2"><Input value={m.name} onChange={(e) => handleMilestoneChange(m.id, "name", e.target.value)} placeholder={`e.g. ${getOrdinal(index + 1)} Payment`}/></td>
              <td className="p-2"><Input type="number" step="0.01" value={m.amount} max={maxAllowed} onChange={(e) => handleMilestoneChange(m.id, "amount", e.target.value)} onFocus={handleFocus}/></td>
              <td className="p-2"><Input type="date" value={m.due_date || ""} min={today} onChange={(e) => handleMilestoneChange(m.id, "due_date", e.target.value)} className={!m.due_date ? 'border-red-500 focus-visible:ring-red-500' : ''}/></td>
        ...
        <tr className="border-t bg-gray-50 font-medium"><td className="p-2">Total</td><td className="p-2">{formatToIndianRupee(totalAmount)}</td><td></td><td className="p-2 text-center">{(totalPercentage || 0).toFixed(0)}%</td><td></td></tr>
      </tbody>
    </table>
  );
  ...
  const renderDeliveryTable = () => (
    <table className="w-full text-sm">
      <thead className="bg-gray-100"><tr><th className="p-2 text-left font-medium w-2/5">TERM</th><th className="p-2 text-left font-medium w-1/4">PERCENTAGE</th><th className="p-2 text-left font-medium w-1/4">AMOUNT</th></tr></thead>
      <tbody>
          {milestones.map((m, index) => {
              let maxPercent = 100;
              if (index === 1) { const advancePercent = milestones[0].enabled ? (milestones[0].percentage || 0) : 0; maxPercent = 100 - advancePercent; }
              return (<tr key={m.id} className="border-t">
                  <td className="p-2 flex items-center gap-2"><Checkbox id={m.id} checked={m.enabled} disabled={index === 2} onCheckedChange={(checked) => handleEnableTerm(m.id, !!checked)} /><Label htmlFor={m.id} className="font-normal cursor-pointer">{m.name}</Label></td>
                  <td className="p-2"><Input type="number" value={m.enabled ? m.percentage : ""} onChange={e => handleMilestoneChange(m.id, 'percentage', e.target.value)} onFocus={handleFocus} placeholder="%" disabled={!m.enabled} max={maxPercent} readOnly={index === 2} /></td>
                  <td className="p-2 font-medium">{m.enabled ? formatToIndianRupee(m.amount) : "Rs.0"}</td>
              </tr>)
          })}
          <tr className="border-t bg-gray-50 font-semibold"><td className="p-2">Total</td><td className="p-2">{(totalPercentage || 0).toFixed(0)}%</td><td className="p-2">{formatToIndianRupee(totalAmount)}</td></tr>
      </tbody>
    </table>
  );

  return (
    <Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="sm:max-w-2xl">{step === "select" ? renderSelectStep() : renderDetailsStep()}</DialogContent></Dialog>
  );
};
...
//          // --- MODIFIED: Ensure initial amounts are also correctly rounded ---
//         const sanitizedTerms = initialData.terms?.map(term => ({
//           ...term,
...
//     // --- MODIFIED: Use rounded values for accurate pending amount ---
//     return roundToTwo(poAmount - totalAmount);
//   }, [poAmount, totalAmount]);


// // --- NEW: Validation rule based on the pending amount ---
// // This allows confirmation if the pending amount is less than 1 but not negative.
// const isTotalAmountValid = useMemo(() => {
//   return pendingAmount < 1 && pendingAmount >= 0;
// }, [pendingAmount]);
//    // 2. Validates if all 'Credit' milestones have a due date.
//   const areCreditDatesValid = useMemo(() => {
//     if (type !== 'Credit') return true;
//     return milestones.every(m => m.enabled && m.due_date);
//   }, [type, milestones]);
...
//   // --- MODIFIED: Use the new amount-based validation ---
// const isFormValid = useMemo(() => {
//   return isTotalAmountValid && areCreditDatesValid;
// }, [isTotalAmountValid, areCreditDatesValid]);

// // --- MODIFIED: This check is now simpler and more accurate ---
...
//   // ===// Inside the PaymentTermsDialog component

//   // --- ADD THIS HELPER FUNCTION ---
//   // This function converts a number into its ordinal form (e.g., 1 -> 1st, 2 -> 2nd).
...
//       // 1. Calculate the number for the new payment. It will be one more than the current number of milestones.
//       const newPaymentNumber = milestones.length + 1;

//       // 2. Use our helper function to get the ordinal string (e.g., "2nd", "3rd").
//       const ordinalString = getOrdinal(newPaymentNumber);

//       // 3. Construct the default name.
//       const newPaymentName = `${ordinalString} Payment`;

//       // 4. Create the new milestone with the generated name.
//       const newId = `term_${Date.now()}`;
//       setMilestones([
//         ...milestones,
//         {
//           id: newId,
//           name: newPaymentName, // Use the new dynamic name
//           amount: 0,
//           percentage: 0,
//           due_date: "",
//           enabled: true,
...
//           if (!checked)
//             return { ...m, enabled: false, percentage: 0, amount: 0 };
...
//               milestones.reduce((sum, ms) => (ms.id !== id ? sum + ms.amount : sum), 0)
//             );
...
//             newMilestone.amount = roundToTwo(newAmount);
//             newMilestone.percentage =
//               poAmount >=0 ? (newMilestone.amount / poAmount) * 100 : 0;
//           }
...
//             newMilestone.amount = roundToTwo((newPercentage / 100) * poAmount);
//           }
//           return newMilestone;
//         }
//         return m;
...
//                <span>
//                 {isOverBudget
//                   ? `Total cannot exceed 100%. Current: ${(totalPercentage || 0).toFixed(2)}%`
//                   : "The total percentage must be exactly 100% to confirm."}
//               </span>
//             </div>
//           )}
//           {/* --- NEW: Error message for missing due dates in Credit mode --- */}
//           {!areCreditDatesValid && type === 'Credit' && (
//              <div className="flex items-center gap-2 text-xs text-red-500">
//                <span className="h-4 w-4 flex items-center justify-center rounded-full border border-red-500">!</span>
//                <span>A due date is required for every 'Credit' payment term.</span>
//             </div>
//           )}
//         </div>
//       </div>
...
//                   }
//                   // --- NEW: Visual cue for invalid state ---
//                   className={!m.due_date ? 'border-red-500 focus-visible:ring-red-500' : ''}
...
//                 id={m.id}
//                 checked={m.enabled}
//                 onCheckedChange={(checked) => handleEnableTerm(m.id, !!checked)}
//               />
