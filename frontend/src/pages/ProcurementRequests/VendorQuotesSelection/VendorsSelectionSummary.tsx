...
import { useFrappeDocumentEventListener, useFrappePostCall,useFrappeUpdateDoc } from "frappe-react-sdk";
import memoize from 'lodash/memoize';
import { ArrowBigUpDash, CheckCheck, ListChecks, SendToBack, Undo2,CirclePlus } from "lucide-react";
...
const savePaymentTerms = async (): Promise<boolean> => {
    ...
    try {
        ...
        toast({
            ...
            description: "Payment terms have been successfully saved to the Procurement Request.",
            variant: "success",
        });
 ...
 const handleConfirmPaymentTerms = (vendorId: string, data: VendorPaymentTerm) => {
        ...
        <div className="p-6 rounded-lg bg-green-50 border border-green-200">
            ...
            <dl className="space-y-4">
                {Object.entries(vendorWiseApprovalItems).map(([vendor, { items, total, totalInclGst }]) => {
                    ...
                    <div key={vendor}>
                         <dt className="flex justify-between items-center text-sm border-b border-grey-200 font-medium text-red-500 pb-2">
                                        <p>Vendor: <span className="font-semibold text-red-600">{getVendorName(vendor)}</span></p>
                    
                                        <div>
                                            {termsForVendor ? (
                                                // --- If terms exist, show the summary and an Edit button ---
                                                <div className='flex items-center gap-2'>
                                                    <RenderPaymentTermsSummary terms={termsForVendor} />
                                                    <Button 
                                                        variant="link" 
                                                        size="sm" 
                                                        onClick={() => setEditingVendor({ id: vendor, name: getVendorName(vendor), total: totalInclGst })}
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => setEditingVendor({ id: vendor, name: getVendorName(vendor), total: totalInclGst })}
                                                >
                                                    <CirclePlus className="mr-2 h-4 w-4 flex-shrink-0" /> Add Payment Terms
                                                </Button>
                                            )}
                                        </div>
    ...
    <ul className="list-disc space-y-1 text-gray-800">
        {items.map((item) => (
            <li key={item.item_id} className="text-sm">
                {item.item_name}
                {item.make && (<span className="text-gray-500 italic ml-1">({item.make})</span>)}
                <span className="mx-1">-</span> {item.quantity} x {formatToIndianRupee(item.quote)}
                <span className="mx-1 text-gray-500">+</span> {item.tax}% GST
                <span className="mx-1">=</span> <span className="font-medium">{formatToIndianRupee(item.amount)}</span>
                {item?.potentialLoss && (<span className="block text-xs text-red-600 mt-0.5">Potential Loss: {formatToIndianRupee(item.potentialLoss)}</span>)}
            </li>
        ))}
    </ul>
    ...
    */}
    {(() => {
        ...
        if (termsForThisVendor) {
            ...
            return <PaymentTermsDetailsDisplay terms={termsForThisVendor} />;
        }
              ...
              <div className="flex flex-col justify-end items-end mr-2 my-4">
                        <Dialog>
                            <DialogTrigger asChild>
                                <Button className="flex items-center gap-1"disabled={!allVendorsHaveTerms || sendForApprCallLoading}>
                ...
                {!allVendorsHaveTerms && (
                    <p className="text-xs text-red-500 mt-2">Please add payment terms for all vendors before proceeding.</p>
                )}
            </div>
...
const RenderPaymentTermsSummary: React.FC<{ terms: VendorPaymentTerm }> = ({ terms }) => {
    // If for some reason terms are missing, show a fallback.
    if (!terms.terms || terms.terms.length === 0) {
      return <span className="text-xs font-normal text-gray-600">{terms.type}</span>;
    }
      ...
      <div className="flex items-center gap-2 text-xs font-normal text-gray-600">
        ...
        {terms.terms.map((milestone, index) => (
          <React.Fragment key={milestone.id}>
            ...
            {index < terms.terms.length - 1 && <span className="text-gray-300">|</span>}
          </React.Fragment>
        ))}
      </div>
    );
...
//                 {Object.entries(vendorWiseApprovalItems).map(([vendor, { items, total, totalInclGst }]) => (
//                     <div key={vendor}>
//                         <dt className=" flex justify-between text-sm border-b border-grey-200 font-medium text-red-500">
...
//                                                                             variant="outline"
//                                                                             size="sm"
//                                                                            className="w-full text-primary border-primary justify-start hover:text-white-600 hover:border-white-600 hover:bg-red-600"
