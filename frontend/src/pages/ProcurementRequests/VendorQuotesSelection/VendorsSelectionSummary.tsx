...
export const VendorsSelectionSummary: React.FC = () => {
  ...
  useEffect(() => {
    if (Object.keys(paymentTerms).length > 0) {
      localStorage.setItem(`paymentTermsDraft_${prId}`, JSON.stringify(paymentTerms));
    }
  }, [paymentTerms, prId]);
      ...
      memoize(
        (itemId: string): number => {
          ...
          const allQuotesForItem =
            orderData.rfq_data.details[itemId].vendorQuotes;
  ...
  const savePaymentTerms = async (): Promise<boolean> => {
    ...
    try {
      ...
      toast({
        ...
        description: "Could not save the payment terms.",
        variant: "destructive",
      });
      return false;
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.message.status === 200) {
        ...
        toast({
          title: "Failed!",
          description: response.message.error || "Unknown error",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const itemIdsToFetch = useMemo(
    () =>
      getItemListFromDocument(orderData)
  ...
  const generateActionSummary = useCallback(() => {
    ...
    orderData?.order_list.forEach((item: ProcurementRequestItemDetail) => {
      ...
      if (!vendor || !quote) {
        ...
        // 2. Lowest Quote in RFQ (L1)
        const lowestRate = getLowest(item.item_id);
        const lowestQuotedAmount = (lowestRate !== Infinity) ? (lowestRate * quantity) : 0;

        // 3. Benchmark Selection (MODIFIED)
        // To show "Budget Savings", we prioritize Target Amount.
        // If Target exists, use it. Else, use Lowest Quote.
        let benchmarkAmount = 0;
        if (targetAmount > 0) {
            benchmarkAmount = targetAmount; // Prioritize Target for savings calc
        } else if (lowestQuotedAmount > 0) {
            benchmarkAmount = lowestQuotedAmount; // Fallback to L1 if no target
        }
      ...
      <div className="flex items-center justify-center h-[90vh]">
        <div className="bg-white shadow-lg rounded-lg p-8 max-w-lg w-full text-center space-y-4">
          ...
          <p className="text-gray-600 text-lg">
            PR: <span className="font-medium text-gray-900">{orderData?.name}</span> is not In Progress.
          </p>
          <button
      ...
      <div className="flex flex-col gap-4">
        {Object.keys(vendorWiseApprovalItems).length > 0 && (
          <div className="space-y-3">
            {/* --- ADDED: Header Text Section --- */}
            <div className="pl-1 space-y-1">
                <div className="flex items-center gap-2">
                    <ListChecks className="h-5 w-5 text-primary" />
                    <h3 className="text-lg font-semibold text-gray-900">Items for Approval</h3>
                </div>
                 ...
                 <div className="flex flex-col gap-4 mt-4">
                  <div className="flex flex-col gap-2 text-start">
                    ...
                    <TextArea
                      ...
                      onChange={(e) => setComment({ ...comment, approving: e.target.value })}
                    />
                  </div>
                  {allDelayedItems.length > 0 && (
                    <div className="flex flex-col gap-2 text-start">
                      ...
                      <TextArea
                         ...
                         onChange={(e) => setComment({ ...comment, delaying: e.target.value })}
                      />
                    </div>
                  )}
                 </div>
