...
export const ApproveSelectVendor: React.FC = () => {
  ...
  const {
    ...
    error: userError,
  } = useUsersList(); // Keep for owner display
        ...
        cell: ({ row }) => {
          ...
          const isNew = notifications.find(
            (item) =>
              item.docname === prId &&
              item.seen === "false" &&
              item.event_id === "pr:vendorSelected" // Assuming this event ID
          );
          return (
            <div
              role="button"
            ...
            >
              ...
              <Link
                ...
                to={`/purchase-orders/${prId}?tab=Approve PO`}
              >
        ...
        cell: ({ row }) => {
          ...
          return (
            <p className="font-medium pr-2">
              {total === 0 ? "N/A" : formatToRoundedIndianRupee(total)}
            </p>
          );
        },
        size: 150,
