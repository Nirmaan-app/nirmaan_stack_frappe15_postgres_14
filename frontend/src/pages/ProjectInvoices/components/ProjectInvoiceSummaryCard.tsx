...
export const ProjectInvoiceSummaryCard: React.FC<ProjectInvoiceSummaryCardProps> = ({
    aggregates,
    isAggregatesLoading,
    totalCount,
    columnFilters,
    ...
    const getTitle = () => {
        if (projectName) return `Invoice Summary — ${projectName}`;
        if (customerName) return `Invoice Summary — ${customerName}`;
            ...
            <Card className="border-0 shadow-sm bg-gradient-to-br from-slate-50 to-white dark:from-slate-900 dark:to-slate-800">
                <CardContent className="p-6">
                    <div className="flex justify-center items-center h-16">
                        <TailSpin height={28} width={28} color="#0d9488" />
                    </div>
                </CardContent>
            </Card>
        );
...
export default ProjectInvoiceSummaryCard;
