...
export const AllProjectInvoices: React.FC<{
  ...
  const [invoiceToDelete, setInvoiceToDelete] = useState<ProjectInvoice | null>(
    null
  );
  ...
  const getCustomerName = useCallback(
    memoize(
      (custId?: string) =>
        customers?.find((p) => p.name === custId)?.company_name ||
        custId ||
        "--"
    ),
    [customers]
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong className="px-1 font-semibold text-foreground">
