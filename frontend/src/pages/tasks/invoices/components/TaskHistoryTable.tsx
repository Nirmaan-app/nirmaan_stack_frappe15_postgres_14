...
export const TaskHistoryTable: React.FC = () => {
  ...
  const [attachmentsData, setAttachmentsData] = React.useState<
    NirmaanAttachment[] | string | undefined
  >(undefined);
  ...
  } = useFrappePostCall<NirmaanAttachment[]>(
    ...
    const performFetch = async () => {
      ...
      if (attachmentIds && attachmentIds.length > 0) {
        ...
        try {
          ...
          console.error(
            "An error occurred while calling fetchAttachments:",
            err
          );
        }
      }
  ...
  const staticFilters = useMemo(
    () => getInvoiceTaskStaticFilters("", role, user_id),
    [role, user_id]
  );
  ...
  const invoiceTaskSearchableFeilds = useMemo(
    () =>
      INVOICE_TASK_SEARCHABLE_FIELDS.concat([
        {
          value: "assignee",
          label: "Actioned By",
