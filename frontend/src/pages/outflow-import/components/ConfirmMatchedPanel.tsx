// src/pages/outflow-import/components/ConfirmMatchedPanel.tsx
...
interface Props {
    ...
    /**
     * Whether this panel is the thing on screen.
     *
     * ⚠️ IT IS NOT COSMETIC — three behaviours hang off it, unchanged from when it was the dialog's
     * `open`: the fetch is withheld while false, the selection and results RESET on the way out
     * (a reused surface must not open showing the last run's results), and the ready rows are
     * re-seeded on the way in. A host that mounts and unmounts the panel could pass a constant
     * `true`; one that keeps it mounted must pass the truth.
     */
    active: boolean;
    /** Cancel, or Close on the results view. */
    onClose: () => void;
    onSettled: () => Promise<void> | void;
    /**
     * Fired while the settle loop is running, so a host can refuse to be dismissed mid-write.
     *
     * ⚠️ THE HOST OWNS THE REFUSAL BECAUSE ONLY THE HOST HAS A DISMISS. A dialog blocks its
     * `onOpenChange`; a wizard step blocks its Back and its Finish. The panel cannot know which.
     */
    onRunningChange?: (running: boolean) => void;
    /**
     * The chrome this panel is rendered inside.
     *
     * ⚠️ THE HEADING TEXT STAYS IN THE PANEL AND ONLY THE ELEMENT CHANGES (slice CF/S6). The words
     * depend on internal state — "Confirm matched transfers" becomes "Confirmation results" once the
     * loop has run — so a host cannot render them without duplicating that state. But a Radix dialog
     * needs a real `DialogTitle` for accessibility, and a wizard step must NOT emit one. Passing the
     * elements in keeps one copy of the words and lets each host supply its own container.
     */
    Title?: ElementType;
    Description?: ElementType;
    Footer?: ElementType;
    /**
     * What to say when nothing is confirmable.
     *
     * ⚠️ IT IS THE HOST'S TO SET BECAUSE THE HOSTS SEARCH DIFFERENT POPULATIONS (found in the CF/S7
     * browser walk, not by a test). The summary dialog is scoped to the screen's filters; the
     * wizard's step 4 is unfiltered and says so in the line directly above. The shipped sentence —
     * "in this import" — was therefore a flat contradiction of the sentence above it.
     */
    emptyNote?: string;
    /**
     * Render the panel's own Cancel button.
     *
     * ⚠️ FALSE IN THE WIZARD, AND NOT MERELY TO AVOID TWO BUTTONS. At step 4 the transfers are
     * ALREADY IMPORTED, so "Cancel" reads as an offer to undo them — the opposite of the truth, on
     * the one screen where a reviewer is deciding whether their work was saved. The wizard's own
     * "Finish later" is the honest exit, and it sits beside a sentence saying the rows are in.
     */
    showCancel?: boolean;
}
 ...
 */
export const ConfirmMatchedPanel = ({
    filters: scope,
    active: open,
    onClose,
    onSettled,
    onRunningChange,
    // Plain elements by default, so a host with no chrome gets something sensible rather than
    // nothing. The dialog overrides all three with the Radix ones.
    Title = "h3",
    Description = "p",
    Footer = "div",
    emptyNote = "Nothing is ready to confirm in this import.",
    showCancel = true,
}: Props) => {
    /**
    ...
    /**
     * Tell the host while a settle loop is in flight.
     *
     * ⚠️ AN EFFECT ON THE STATE, NOT A CALL BESIDE EVERY `setRunning`. There are three of those and
     * one is in a `finally`-shaped reset; a hand-placed notification would eventually miss one, and
     * the failure is a host that stays locked after the run finished — or worse, one that lets
     * itself be dismissed mid-write.
     */
    useEffect(() => {
    ...
    return (
        <>
                    ...
                    <div className="max-h-[50vh] space-y-4 overflow-y-auto">
                        ...
                        {ready.length === 0 && (
                            <p className="py-6 text-center text-sm text-muted-foreground">
                                {/* ⚠️ THE HOST OVERRIDES THIS, BECAUSE THE HOSTS HAVE DIFFERENT
                                    SCOPES (found in the CF/S7 browser walk). The wizard's step 4 is
                                    unfiltered — every open import — and says so in a line directly
                                    above this one, so the shipped "in this import" contradicted the
                                    sentence it sat under. Nothing about the fetch changed; only the
                                    sentence naming what was searched. */}
                                {emptyNote}
                            </p>
                        )}
                ...
                <Footer className="flex flex-col-reverse sm:flex-col sm:items-stretch sm:gap-3">
