...
const ApproveAmendPO = () => {
    ...
    const { data: po_data, isLoading: po_loading, error: po_error } = useFrappeGetDoc<ProcurementOrdersType>("Procurement Orders", po, `Procurement Orders ${po}`);
    ...
    useEffect(() => {
        if (po_data && !po_loading) {
            setProject(po_data?.project)
            setOwner(po_data?.owner)
        }
    }, [po_data, po_loading, project, owner])

    // console.log("within 1st component", owner_data)
    if (po_loading || project_loading || owner_loading) return <h1>Loading...</h1>
    if (po_error || project_error || owner_error) return <h1>Error</h1>
    return (
        <ApproveAmendPOPage po_data={po_data} project_data={project_data} owner_data={owner_data == undefined ? { full_name: "Administrator" } : owner_data} />
    )
}
...
const ApproveAmendPOPage = ({ po_data, project_data, owner_data }: ApproveAmendPOPageProps) => {
    ...
    const { data: universalComments } = useFrappeGetDocList("Nirmaan Comments", {
        fields: ["*"],
        filters: [["reference_name", "=", po_data.name]],
        orderBy: { field: "creation", order: "desc" }
    })
    ...
    useEffect(() => {
        if (usersList) {
            let ids: any = usersList.map((user) => user?.name)
    ...
    const handleCommentChange = (item: any, e: any) => {
    ...
    const handleUniversalCommentChange = (e: any) => {
    ...
    //         let mod_pr_data = { ...po_data, order_list: JSON.parse(po_data?.order_list) }
    ...
    const handleApprove = async () => {
        try {
            ...
            const res = await updateDoc('Procurement Orders', po_data.name, {
            ...
            toast({
                title: "Success!",
                description: `P0: ${res?.name} is successfully Approved!`,
                variant: "success"
            });
        ...
        try {
            ...
            if (universalComment) {
                await createDoc("Nirmaan Comments", {
                    ...
                    comment_by: userData?.user_id,
                    content: universalComment,
                    subject: "rejecting po amend"
                })
            }
            await mutate("ApprovePO,PRListMutate")
            toast({
                title: "Success!",
                ...
                <div className="flex-1 md:space-y-4">
                    ...
                    <Card className="flex flex-wrap md:grid md:grid-cols-4 gap-4 border border-gray-100 rounded-lg p-4">
                        <div className="border-0 flex flex-col justify-center max-sm:hidden">
                            ...
                            <p className="text-left font-bold py-1 font-bold text-base text-black">{formatDate(po_data?.creation)}</p>
                        </div>
                    ...
                    <Card className="p-4 border border-gray-100 rounded-lg">
                        ...
                        {orderData?.order_list.list.length === 0 && <div className="text-sm">No Items to display, please click on "Undo" button to recover the deleted items or add at least an item to enable the "Approve" or "Reject" button</div>}
                    ...
                    <div className="flex gap-4 justify-end items-end mt-4">
                        <Button disabled={!orderData.order_list.list.length} variant="secondary" className="" onClick={() => {
                            setPage('summary')
