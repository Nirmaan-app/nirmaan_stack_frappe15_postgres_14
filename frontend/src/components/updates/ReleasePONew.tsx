...
export const ReleasePONew = ({ not }) => {
  ...
  const {
    data: procurement_order_list,
    isLoading: procurement_order_list_loading,
    error: procurement_order_list_error,
    mutate: mutate,
  } = useFrappeGetDocList(
    "Procurement Orders",
    {
  ...
  const {
    ...
    error: vendor_error,
  } = useFrappeGetDoc("Vendors", vendor, vendor === "" ? null : undefined);
  ...
  const handleUnmerge = (po) => {
    if (orderData) {
      const updatedList = orderData.order_list.list.filter(
  ...
  useEffect(() => {
    if (procurement_order_list && orderId) {
      const curOrder = procurement_order_list.find(
        (item) => item.name === orderId
      );
      if (curOrder?.status === "PO Approved") {
        const mergeablePOs = procurement_order_list.filter(
          (item) =>
            item.project === curOrder?.project &&
            item.vendor === curOrder?.vendor &&
            item.status === "PO Approved" &&
            item.name !== orderId
        );
        setMergeablePOs(mergeablePOs);
        if (curOrder?.merged === "true") {
          const mergedPOs = procurement_order_list.filter(
            (po) => po?.merged === orderId
          );
  ...
  const {
    ...
    isCompleted: update_submit_complete,
    error: update_submit_error,
  } = useFrappeUpdateDoc();
  ...
  if (
    procurement_order_list_loading ||
  ...
  if (procurement_order_list_error || address_list_error || vendor_error)
    return <h1>Error</h1>;
  if (!not && !["PO Approved", "Merged"].includes(orderData?.status))
    return (
          ...
          <p className="text-gray-600 text-lg">
            ...
            <span className="font-medium text-gray-900">
              {orderData?.modified_by === "Administrator"
                ...
                : getUserName(orderData?.modified_by)}
            </span>
            !
          </p>
          <button
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-4 pb-4">
            <div className="flex-col">
              ...
              <div className="flex-1 py-2">
                ...
                <p className="ml-2">
                  <Badge variant={"gray"}>Total agregated percentages: </Badge>{" "}
                  {advance +
                    materialReadiness +
                    afterDelivery +
                    xDaysAfterDelivery}{" "}
                ...
                <p className="ml-2 my-2">
                  ...
                  <strong>
                    Total agregated percentage must sum up to 100% in order to
                    enable save button!
                  </strong>{" "}
                </p>
              </div>
        ...
        <Layout className="bg-white">
          <div className="flex">
            <Button
              ...
              icon={
                collapsed ? (
                  <NotebookPen className="hover:text-primary/40" />
                ) : (
                  <ArrowLeftToLine className="hover:text-primary/40" />
                )
              }
            ...
            <Content
              className={`${
                collapsed ? "md:mx-10 lg:mx-32" : ""
              } my-4 mx-2 flex flex-col gap-4 relative`}
            >
              <div className="absolute right-0 -top-20 max-md:-top-14 flex items-center gap-4">
                ...
                >
                  {orderData?.status === "Partially Delivered"
                    ? "Delivered"
                    : orderData?.status}
                </Badge>
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            You are seeing this because of some validation
                            checks from the payment terms inputs are not
                            fulfilled, please go the editing section and do the
                            needful to proceed with printing!
                          </DialogDescription>
                        </DialogHeader>
                      </DialogContent>
                    </Dialog>
                                ...
                                <Table>
                                  ...
                                  <TableBody>
                                    <TableRow key={orderData.name}>
                                      ...
                                      <TableCell>
                                        {
                                          orderData.order_list.list.filter(
                                            (i) => !i?.po
                                          )?.length
                                        }
                                      </TableCell>
              ...
              <div
                className={`w-full border rounded-lg h-screen overflow-y-scroll`}
              >
                              ...
                              <div>
                                ...
                                <div className="pt-2">
                                  <div className="text-sm font-normal text-gray-900 text-left">
                                    ...
                                    &nbsp;&nbsp;&nbsp;
                                    <i>{orderData?.creation?.split(" ")[0]}</i>
                                  </div>
                                  <div className="text-sm font-normal text-gray-900 text-left">
                                    <span className="text-gray-600 font-normal">
                                      Project Name:
                        ...
                        {[
                          ...new Map(
                            orderData?.order_list?.list.map((item) => [
                              ...
                              {
                                ...item,
                                quantity: orderData.order_list.list
                                  .filter(
                                    ({ item: itemName }) =>
                                      itemName === item.item
                                  )
                                        ...
                                        <Dialog>
                                          ...
                                          <DialogContent>
                                            <DialogHeader>
                                              ...
                                              <DialogDescription>
                                                You are seeing this because of
                                                some validation checks from the
                                                payment terms inputs are not
                                                fulfilled, please go the editing
                                                section and do the needful to
                          ...
                          <DialogContent>
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                You are seeing this because of some validation
                                checks from the payment terms inputs are not
                                fulfilled, please go the editing section and do
                                the needful to proceed with printing!
                              </DialogDescription>
                            </DialogHeader>
                          </DialogContent>
                        </Dialog>
                      ...
                      <div className="space-y-6">
                        <div className="bg-green-100 p-4 rounded-lg border border-green-200">
                          ...
                          <ul className="list-disc list-inside space-y-1 text-sm text-green-700">
                            <li>
                              Ensure all items are properly packed and labeled.
                            </li>
                            <li>
                              Verify the delivery address and contact
                              information.
                            </li>
                            <li>
                              Attach any necessary documentation to the package.
                            </li>
                          </ul>
                        </div>
                        <div>
                          ...
                          <p className="text-base text-gray-600 mb-4">
                            Please enter the delivery person's Name and Contact
                            Number:
                          </p>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                        ...
                        <div className="flex justify-end mt-4">
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                <AlertDialogTitle>
                                  Is this order dispatched?
                                </AlertDialogTitle>
                                <AlertDialogDescription>
                                  This action will create a delivery note for
                                  the project manager on site. Are you sure you
                                  want to continue?
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              {clicked ? (
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogDescription className="space-y-2">
                                  <div>
                                    Please be informed that, the following are
                      ...
                      <div className="flex justify-end">
                        {["PO Approved"].includes(orderData?.status) ? (
                          orderData?.merged !== "true" ? (
                            <ShadButton
                              onClick={() =>
                                document
                                  .getElementById("amendAlertTrigger")
                                  ?.click()
                              }
                              className="flex items-center gap-1"
                            >
                        ...
                        <Sheet>
                          <SheetTrigger asChild>
                            <button className="hidden" id="amendAlertTrigger">
                              trigger
                            </button>
                          </SheetTrigger>
                          <SheetContent className="overflow-auto">
                            <>
                              <div className="pb-4 text-lg font-bold">
                                Amend:{" "}
                                <span className="text-red-700">{orderId}</span>
                              </div>
                              {/* PENDING CARD */}
                              <Card className="p-4">
                      ...
                      <div className="flex justify-end">
                        {["PO Approved"].includes(orderData?.status) ? (
                          ...
                          orderData?.merged !== "true" ? (
                            <ShadButton
                              onClick={() =>
                                document.getElementById("alertTrigger")?.click()
                              }
                              className="border-primary flex items-center gap-1"
                            >
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger>
                            <button className="hidden" id="alertTrigger">
                              trigger
                            </button>
                          </AlertDialogTrigger>
                          <AlertDialogContent>
