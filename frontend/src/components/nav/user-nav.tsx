        ...
        <DropdownMenu>
            ...
            {!isMobileMain ? (
              ...
              >
                <Avatar className="h-8 w-8 rounded-lg">
                  ...
                  <AvatarFallback className={`rounded-lg ${state === "collapsed" && "bg-transparent" }`}>{generateFallback(userData.full_name)}</AvatarFallback>
                </Avatar>
          ...
          >
            <DropdownMenuLabel className="p-0 font-normal">
              ...
              <Avatar className="h-8 w-8 rounded-lg">
                <AvatarImage src={userData.user_image} alt={userData.full_name} />
                <AvatarFallback className="rounded-lg">{generateFallback(userData.full_name)}</AvatarFallback>
              </Avatar>
                <div className="grid flex-1 text-left text-sm leading-tight">
                  <span className="truncate font-semibold">{userData.full_name}</span>
                  <span className="truncate text-xs">{userData.user_id}</span>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={logout} className="flex gap-1 items-center cursor-pointer">
