...
export const ProcurementOrder = () => {
  ...
  const {
    data: vendor_category_list,
    isLoading: vendor_category_list_loading,
    error: vendor_category_list_error,
    mutate: vendor_category_mutate,
  } = useFrappeGetDocList(
    "Vendor Category",
    {
      fields: ["*"],
  ...
  const { data: quote_data } = useFrappeGetDocList(
    "Approved Quotations",
    {
      fields: ["*"],
      limit: 10000,
  ...
  useEffect(() => {
    if (universalComments) {
      const comments = universalComments?.filter((cmt) =>
        ["approving pr", "creating pr"].includes(cmt.subject)
      );
  ...
  const handleSubmit = async () => {
    ...
    orderData.procurement_list.list.forEach((item) => {
      ...
      selectedCategories[curCategory].forEach((ven) => {
        ...
        const new_quantity = new_procurement_list?.list.find(
                                ...
                                <TableCell>
                                  <div className="inline items-baseline">
                                    ...
                                    {item.comment && (
                                      <HoverCard>
                                        <HoverCardTrigger>
                                          <MessageCircleMore className="text-blue-400 w-6 h-6 inline-block ml-1" />
                                        </HoverCardTrigger>
              ...
              <div>
                <div className="flex m-2 justify-between">
                  <div>
                    <HoverCard>
                      <HoverCardTrigger>
                        <div className="text-xl font-bold py-2 text-red-700 underline">
                          {cat.name}
                        </div>
                      </HoverCardTrigger>
                        ...
                        <SheetTitle>
                          <div className="flex-1">
                            ...
                            <p className=" text-xs font-light text-slate-500 p-1">
                              Add a new vendor here with at least{" "}
                            ...
                            <p className=" text-xs font-light text-slate-500 p-1">
                              ...
                              Check if the vendor is already available! If yes,
                              then click on cross at top right, scroll down to
                              add{" "}
                              <span className="text-red-700 italic">
                                {cat.name}
                              </span>{" "}
                ...
                <Select
                  options={getCategoryByName(cat.name)}
            ...
            {loading || update_loading ? (
              ...
              <Button
                disabled={
                  ...
                  (selectedCategories &&
                    Object.values(selectedCategories).some(
                      (arr) => !arr.length
                    )) ||
                  (selectedCategories &&
                    Object.keys(selectedCategories)?.length !==
              ...
              <AccordionTrigger>
                <div className="md:mb-2 text-base md:text-lg px-2  w-full text-left">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-400">
                      Check if you have added a vendor previously and want to
                      update their{" "}
