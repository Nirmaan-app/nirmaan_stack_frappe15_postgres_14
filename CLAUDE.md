...
**Last updated:** 2026-06-05 (UNIFY-ON-(-1) + Slice A review-screen backend -- feat fff26abd; has_human_parent retired, -1 sentinel for parent_index + human_parent, review_screen.py helpers + 3 endpoints; 34 new wizard tests; BACKEND ONLY) **Frontend conventions file: `frontend/CLAUDE.md` (NOT `frontend/.claude/CLAUDE.md`).**
...
|---|---|---|---|
| BoQ Upload & Management | `feature/boq-phase-3` | `frontend/.claude/plans/boq-upload-plan.md` | Phases 1.x (parser, 588 tests) + Phase 3 Module 1a/1b + Module 2a COMPLETE. Parse-run Phase 1 IN PROGRESS -- Bucket-2 COMPLETE (Slice 1 backend feat cb86b92b + Slice 2 frontend feat 21e56963). UNIFY-ON-(-1) + Slice A review-screen backend COMPLETE (feat fff26abd; has_human_parent retired; -1 sentinel convention; review_screen.py -- resolve_effective / check_structural_integrity / append_edit_log_entry + 3 endpoints; 34 new tests). Wizard tests: 202 total (test_parse_run 60 + test_review_screen 34 + test_update_sheet_draft 66 + test_update_boq_draft 7 + test_sheet_preview 23 + test_upload_file 12); frontend slices verified by tsc + Vite build. Full sub-phase history + as-built detail: see boq-upload-plan.md (per-slice sections). Do not duplicate the changelog here. |
...
- `get_review_rows(boq_name, sheet_name)` -- `@frappe.whitelist()` bare (GET-capable). Returns `{rows: [...with effective values...], work_packages: [...]}`. JSON list/dict fields returned as parsed Python objects. URL: `/api/method/nirmaan_stack.api.boq.wizard.review_screen.get_review_rows`
