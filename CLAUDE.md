...
12. **Document Autofill (Gemini):** Document extraction is **Gemini-only** — Google Document AI was fully removed (deleted `services/document_ai.py`, its credentials/processor config, and the `google-cloud-documentai` dep). The extraction seam is a modular package `services/extraction/` — `extract(content, file_ext, settings, doc_kind)` dispatches to a provider (today only `gemini.py`); add a future provider as one module + one registry entry in `__init__.py`. **Two whitelisted endpoints, preserved `(text, entities)` contract:** `api/invoice_autofill.extract_invoice_fields` (doc_kind="invoice") and `api/payment_autofill.extract_payment_fields` (doc_kind="payment"); both consume entities via the shared `helpers.pick_entity` so the 5 frontend dialogs/normalization/persistence are untouched. **Trust is computed, not model-reported** (a generative model self-reports ~100%): `services/extraction/validation.py` does GSTIN **mod-36 checksum**, **amount reconciliation** (net+tax+round_off+other_charges+tcs ≈ total, ₹5 tolerance), and date sanity — three states VALID/INVALID/ABSENT, never conflated. **Autofill** populates present fields + soft-warns on failed checks (submit not blocked); **auto-approve gate 5** (in `api/invoices/_auto_approve.py`) was redefined from "confidence ≥ 0.80" to "deterministic validation passes", and **can't-verify (ABSENT) blocks** → stays Pending (gate 5 owns checksum+reconcile; gates 6/7 own GSTIN cross-match; gate 11 owns future-date — no overlap). **Settings** (`Document AI Settings` singleton — doctype name kept): `provider`, `gemini_model` (default `gemini-3.1-pro-preview`), `gemini_thinking_level` (Gemini 3.x — NOT `thinking_budget`), `gemini_media_resolution`, and `gemini_auth_mode` = **Vertex AI** (default; ADC via `GOOGLE_APPLICATION_CREDENTIALS`/workload identity — **no secret in the DB**, `gcp_project_id`+`gcp_location=asia-south1`) or **API Key** (`gemini_api_key`, encrypted Password, dev/test only). Dep pinned `google-genai>=1.55,<1.56` (1.56+ raises its google-auth floor above frappe's `~=2.40.3`). Patch `v3_0.purge_document_ai_credentials` (registered) deletes the old DocAI `tabSingles` rows incl. the plaintext service-account key. Gemini schema is **nullable / no-required** (a required field makes the model fabricate). See `.claude/context/domain/invoice-autofill.md` for full flow.
...
| **Document Autofill** | `.claude/context/domain/invoice-autofill.md` | Gemini invoice/receipt prefill, modular extraction seam, deterministic validation layer, auto-approve gate 5 |
