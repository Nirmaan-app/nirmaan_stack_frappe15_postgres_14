...
- **Revision carry (owner-locked, ADR-0014 Amendment C + Amendment D):** a revision COMMIT carries **nothing** but the D2 provenance triple (`committed_carry.stamp_revision_provenance`; the stamp must stay — it is how the carry finds its source). Formulas are **hand-declared per sheet**, exactly as in the normal phase, and that declaration gates ONE explicit per-sheet action in the pricing editor (`cross_boq_carry.apply_sheet_carry` — synchronous, atomic) which moves **RATES AND NOTHING ELSE**. **Amendment D (2026-07-23) deleted the annotation carry entirely**: remark / colour / `remark` dismissal / category are never copied between a revision and its original by any seam (a carried remark was indistinguishable from a locally-written one in the Review block, and Overwrite superseded the reviewer's own). Each layer keeps its own first-class write path — only the cross-BoQ copy is gone. The layer engine (`carry_layers` / `plan_layer_counts` / `LAYER_KEYS` / `build_carry_ctx`), the `layers` endpoint parameter and `persist.carry_row_categories` are DELETED — do not reintroduce them. Formulas NEVER carry, in either seam. The source read is **version-pinned** to the original's current committed version, and `revision._carry_counts` is pinned identically — **never pin one without the other** (that divergence is the defect Amendment B W6 was written for).
