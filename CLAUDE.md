...
**Last updated:** 2026-06-07 (Slice C-v2c -- per-row Remarks; BACKEND new `remarks` Small Text field [migrated; column landed as text] + NEW save_review_remark endpoint [frappe.db.set_value ONLY -- never edited_at/edit_log, so the row stays "Original"; 500-char hard guard; blank clears to None; sheet_name VERBATIM] + remarks added to get_review_rows all_fields; FRONTEND blue MessageSquare marker inside the Classification cell [no new column, no totalCols change] + master Show/Hide-all-remarks toggle [default off, mirrors flags] + detail-panel Textarea+500-counter+Save with DEDICATED remarkError + sheet-level "Remarks: N" strip; review_screen tests 90->95 +5; tsc 0 errors; Vite build exit 0 in container) **Frontend conventions file: `frontend/CLAUDE.md` (NOT `frontend/.claude/CLAUDE.md`).**
