...
**`general_specs_sheet` empty-string guard** -- `assemble_mapping_config` reads `boq_doc.general_specs_sheet or ""`. Both `None` and `""` are treated as "no general-specs sheet" -- verified both forms occur in real data. Rule 2 (`sheet_name == general_specs_sheet`) can only fire when the value is a non-empty string.
