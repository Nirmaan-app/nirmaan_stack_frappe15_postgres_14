...
**Last updated:** 2026-06-07 (Slice C-v2b -- inline text-editing for unit + make_model; BACKEND text write-branch [setattr, no float coercion; blank clears to None] + allowed-set adds _TEXT_FIELDS; FRONTEND separate "Edit text" block with DIRECT save [no confirm dialog]; description/row_notes stay read-only; review_screen tests 84->90 +6; tsc 0 wizard errors; Vite build exit 0 in container) **Frontend conventions file: `frontend/CLAUDE.md` (NOT `frontend/.claude/CLAUDE.md`).**
...
- Review edits (human layer, Slice A): `human_classification` (Data), `human_parent` (Int), `edit_log` (JSON list), `edited_by` (Data), `edited_at` (Datetime). `has_human_parent` DOES NOT EXIST (was retired in feat fff26abd -- see -1 sentinel below).
