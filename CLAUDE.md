...
**Last updated:** 2026-06-16 (Phase 5 Slice 3a -- GRID FOUNDATION + doctype rename + column-config snapshot +
...
- Internal write-body (`_commit_one_sheet` -> `_write_grid` + `_write_committed_boq_sheet` + `_commit_node_tree_stub`): PER-SHEET transaction (one trailing `frappe.db.commit()` per sheet, NO savepoints -- the app-wide pattern). `_write_grid` -- VERBATIM (#152) `source_sheet_name` on both the prior-version lookup and the freeze; `commit_version = max(prior)+1`; `is_current=1`; `committed_at` set; prior current frozen -> 0 (exactly one current per (boq, sheet)). `_write_committed_boq_sheet` -- REPLACE semantics (raw `frappe.db.delete` of the prior (boq, sheet) BoQ Sheet + its child work_packages, then create fresh; raw delete because `BoQ Sheet.area_dimensions` is a list-valued JSON field that `delete_doc`'s as_json archive rejects -- see gotcha below); COLUMN-CONFIG SNAPSHOT from the matching draft's `sheet_config` (`column_role_map`/`column_headers`/`area_dimensions` json.dumps'd, + `header_row`/`header_row_count`/`treat_as`); `sheet_order`/`sheet_label`/`work_packages` carried; `treat_as` = master_preamble (general_specs) / data (finalized). `_commit_node_tree_stub` -- TODO(3b) no-op, called only for finalized.
...
**GOTCHA -- list-valued JSON field + delete_doc (Slice 3a, pin):** a JSON field holding a Python LIST (e.g. `BoQ Sheet.area_dimensions`) can be INSERTED via `json.dumps()` (string passes `get_valid_dict`) but Frappe parses it back to a list on load, so any later `doc.save()` OR `delete_doc` (whose as_json archive calls `get_valid_dict`) THROWS "cannot be a list". Use raw `frappe.db.delete` (bypasses the lifecycle/archive) + explicit child-row cleanup to remove such a doc. Reads must tolerate the parsed form (`frappe.parse_json(v) if isinstance(v, str) else v`).
