...
**Last updated:** 2026-06-07 (Slice B2c-fix -- Status column unedited branch null→muted gray "Original" pill; green "Edited" pill + row tint unchanged; tsc 0 wizard errors; Vite build exit 0) **Frontend conventions file: `frontend/CLAUDE.md` (NOT `frontend/.claude/CLAUDE.md`).**
...
- `append_edit_log_entry(existing_log, field, from_val, to_val, user)` -- appends `{field,from,to,by,at}` to the log list. Handles list/JSON-string/None input. Returns new list (does not mutate input). Caller must json.dumps() before save.
...
- `save_review_edit(boq_name, sheet_name, row_index, field, value)` -- `@frappe.whitelist(methods=["POST"])`. Editable fields: human_classification, human_parent, qty_total, rate_supply, rate_install, rate_combined, amount_total, amount_supply, amount_install. Cycle-guard + self-parent guard for human_parent. Appends to edit_log; stamps edited_by + edited_at. Pre-serializes _RESAVE_LIST_JSON_FIELDS before doc.save() (Defect-1 fix). Returns `{ok, row_index, field, from, to, effective}`. URL: `/api/method/nirmaan_stack.api.boq.wizard.review_screen.save_review_edit`
