...
- **BCS — the INTERNAL cost layer of a committed sheet.** BCS records what the work costs US (hand-typed Supply / Installation / Combined cost rates per row on `BoQ Row BCS Rate`) against the BoQ amount we charge the CLIENT; Total Amount and % Profit are ALWAYS computed downstream from those stored rates and never persisted. Full as-built: `.claude/context/domain/boq-backend.md` (storage, endpoints, carry) + `frontend/.claude/context/domain/boq-frontend.md` (the cost block, margin view, summary axis). ⚠️ **NAME COLLISION — READ THIS BEFORE GREPPING.** "BCS" in the **BoQ Rate Master** sections further down means something else entirely (a stored derivation pipeline: discounted product cost plus wastage, no install). Same three letters, unrelated concept, different owner; a search for BCS lands on both and they must never be reconciled with each other. The four invariants below are the ones that are NOT obvious from reading the code:
  ...
  - **THE SCOPE FENCE (owner-locked) — BCS readiness gates BCS CELLS ONLY.** It must **NEVER** be ANDed into `save_cell_price`'s rate gate. **State the failure mode, because that is what makes this load-bearing: get it wrong and an unconfirmed BCS section silently freezes ordinary client-facing pricing in production** — every rate cell read-only, for a reason no one on the screen can see. The BCS write path is deliberately independent of the three client gates (mandatory-formula, priceability, category) for the same reason: someone must be able to cost a job while the amount formulas are still being declared and the rows are still being categorised. A qty-less Preamble IS costable. The asymmetry IS the decision — do not "restore consistency" with `save_cell_price`.
  - **THE EXPORT-LEAK BOUNDARY — BCS cost, every BCS-derived total, and the margin must NEVER reach the client workbook.** `export_priced_workbook` is handed to the CLIENT, so a BCS value appearing in it leaks what the job costs us. The exclusion holds **by construction** — the export reads `BoQ Cell Pricing` and names its fields explicitly, while BCS lives in its own doctype with no `col_letter` at all — and construction is exactly what a refactor can dissolve without noticing. It is pinned only by a standing test, so treat this as the written half of that guard. **Anything that folds BCS onto `BoQ Cell Pricing`, adds a BCS stamping pass, or widens the export's field list breaks it.** ⚠️ **A NEW STORED COST FIELD MUST BE SEEDED INTO THAT GUARD'S FIXTURE, or the guard passes on an empty axis** — an unseeded field has nothing to leak, which is the precise vacuity the guard exists to avoid, and it has happened once already on this layer.
