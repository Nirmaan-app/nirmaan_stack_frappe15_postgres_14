...
grid as READ-ONLY reference; data sheets keep the node-based pricing grid. NO migrate (sheet_disposition already
exists; the new endpoint is a pure read). **(1) `get_committed_state` (commit_gate.py) surfaces `sheet_disposition`**
(grid_only / grid_and_nodes) -- added to its grid-tier `fields=[...]` + the per-sheet return dict (purely additive;
the hub maps by sheet_name, unaffected). **(2) NEW endpoint `get_committed_sheet_grid(boq_name, sheet_name,
committed_version)` (pricing.py, `@frappe.whitelist()` bare, GET-capable, PURE READ):** returns the faithful committed
grid for ONE (boq, sheet_name VERBATIM #152, committed_version) = `{rows: [{row_number, cells}], column_role_map,
column_headers, area_dimensions, header_row, header_row_count}`. Reads grid rows from `BoQ Committed Sheet Grid Row`
(parent = the current grid for that boq+sheet+version, ORDER BY row_order asc) + the column-config snapshot from the
committed `BoQ Sheet`. A NEW `_parse_json_field` guard handles BOTH a json.dumps STRING and an already-parsed dict/list
(`get_value(as_dict)` returns the JSON fields parsed). **THE EMPTY-CONFIG CASE (load-bearing):** the row return is NEVER
gated on a non-empty config -- a general-specs sheet (SOW) has empty role/header maps, its grid ROWS are returned
regardless (the render falls back to raw Excel letters). Guards mirror the other committed reads (throw on
missing/unknown boq/sheet/version, no committed grid). **(3) FRONTEND render fork (`SheetPricingPage.tsx`):** the pure
exported helper `isGridOnlySheet(committedSheets, sheetName)` (PricingGrid.tsx) returns true ONLY for grid_only,
FAILS-TO-FALSE in the indeterminate (committed-state loading) window so a data sheet never briefly renders grid-only.
When grid-only -> fetch `get_committed_sheet_grid` + render the EXISTING `SheetDataGrid` read-only (pagination stubbed),
SUPPRESS the lock banners / save-status chip / Summary toggle+panel / "Save now" / `onSaveRate` / save-error, replace
the editor note with a read-only reference note; the tab strip STAYS. **TESTS:** `test_commit_gate` 19 -> 20
(disposition surfacing); `test_pricing` 36 -> 41 (faithful-grid endpoint incl. the empty-config case + guards); Vitest
64 -> 68 (`isGridOnlySheet`). tsc 3178 (== baseline), 0 in touched; Vite build exit 0. Live cert: SOW v5 -> 39 rows +
empty config. NEW types `CommittedSheetGridResponse` + `CommittedSheetState.sheet_disposition` (boqTypes.ts). Full
detail in boq-upload-plan.md "General-specs faithful-grid read-only view".)
// prior: 2026-06-21 (SINGLE-EDITOR LOCK -- SLICE A (backend: doctype + atomic acquire-on-first-edit) --
