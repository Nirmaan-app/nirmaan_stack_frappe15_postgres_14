...
**Last updated:** 2026-06-07 (Slice B2b -- left-gutter expander column [frozen-left], inline read-only detail panel [original-vs-effective, edit_log history, empty reason slot], Option-B mutual exclusivity with flag accordion, totalCols 5→6, sticky-header fix [solid bg on <th> cells], ClassificationPill restyle [rounded-full soft fill, left-border dropped], aria-label "advisory notes"→"flags"; tsc 0 wizard errors; Vite build exit 0) **Frontend conventions file: `frontend/CLAUDE.md` (NOT `frontend/.claude/CLAUDE.md`).**
