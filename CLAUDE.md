...
**Last updated:** 2026-06-08 (Restructure surface Slice 1a [feat pending] -- FRONTEND-ONLY searchable sheet-view component `SheetSearchView.tsx` [self-contained get_sheet_preview full-sheet load by looping 200-row windows + role->letter join from draft.sheet_config.column_role_map; trim to #/Sl.No/Description/Unit/every-Qty, hide Rate+Amount; cycling description hit-stepper + ported ReviewTree scroll/center/highlight; FINDS+SHOWS only -- no selection/save/modal, all Slice 1b] + a TEMPORARY `_DevSheetSearchHarness.tsx` and throwaway route `upload-boq/_dev-sheetview/:boqId/:sheetName` [both REMOVED in Slice 1b]; SheetDataGrid untouched; NO backend/doctype/schema change; tsc 0 wizard errors + Vite build exit 0 in-container; LIVE-CERT OWED [dev URLs on BOQ-26-00145: Fire%20Fitting, Electrical%20, HVAC%20]; full detail in frontend/CLAUDE.md + boq-upload-plan.md. // prior: Slice C-classification read-only recon ran 2026-06-08; findings pasted to chat-Claude for analysis before build prompt -- NO code change. // prior: Slice C-v2d-fix [feat 7fee7481] -- per-area edit guard now validates the sent `area` against the SHEET'S defined areas [`sheet_config.area_dimensions` via new helper `_get_sheet_area_dimensions`, sheet_name VERBATIM #152] instead of THIS row's existing dict keys; a defined-but-empty area is a valid value-edit target so the dict key is CREATED if absent [flat + nested rate paths; from=None for a new key]; UNDEFINED area -> "not a defined area for this sheet", no area_dimensions -> "no defined areas" [fail loudly]; blank->0.0 key-stays + provenance + structured edit_log + rate_subkey legality all UNCHANGED from C-v2d; BACKEND-ONLY [frontend already sends the correct verbatim area]; live-cert finding: BOQ-26-00145 'HVAC ' row_index 2 had qty_by_area={} but Phase 1/Phase 2 are defined sheet areas; test_review_screen 105->110 +5 [measured; TestSaveReviewEditPerArea setUpClass now declares area_dimensions + a NoDimSheet]; tsc 0 wizard errors in-container; Vite build exit 0 in-container; NO doctype/schema change; RE-LIVE-CERT OWED -- bench restart + re-test HVAC qty edit; rate-editing live-cert still owed against a Pattern-2-rate vehicle) // prior: 2026-06-08 (Slice C-v2d [feat cd2cc156] -- per-area numeric editing [qty/amount/rate] via an EXTENDED `save_review_edit` [new optional `area` + `rate_subkey` params; area is None == prior flat path]; FLAT one-hop qty_by_area/amount_by_area + NESTED two-hop rate_by_area [`rate_subkey` from `_LEGAL_RATE_SUBKEYS = frozenset(_RATE_ROLE_TO_KIND.values())`, REUSED from parser not copied]; bare-dict read-modify-write riding the existing `_RESAVE_LIST_JSON_FIELDS` list-sibling pre-serialization [the REASON it EXTENDS the endpoint not a fresh path]; blank -> 0.0 with the area key KEPT [never deleted]; STRUCTURED edit_log keying [optional area+rate_subkey on the entry; old flat entries omit them, stay valid]; validation rejects nonexistent-area / missing-rate_subkey / illegal-rate_subkey / non-per-area-field-with-area; `append_edit_log_entry` + return shape gain area/rate_subkey; FRONTEND ReviewTree "Edit per-area values" block mirroring the C-v2 confirm-dialog pattern [openAreaConfirm -> same AlertDialog -> confirmValueSave sends area+rate_subkey; row flips to "Edited"] + rate_by_area TS type fix flat->nested [`RateByAreaCell`; no static read site broke] + EditLogEntry/SaveReviewEditResponse area/rate_subkey + Obs A edit_log render LATEST-FIRST; test_review_screen 95->105 [+10, measured this run; total wizard 276 DERIVED, full suite not re-run]; tsc 0 wizard errors in-container; Vite build exit 0 in-container; NO doctype/schema change; LIVE-CERT NOT YET DONE [rate-editing live-cert OWED against a Pattern-2-rate vehicle -- Electrical has no nested rate data; BOQ-26-00145 doubly-dirtied by probes, re-parse before live-cert]) // prior: 2026-06-08 (Slice C-v2c-polish [fix ae9dcff2] -- remark marker now ALWAYS-VISIBLE [gate simplified to just `hasRemark`]; master Show/Hide-all-remarks toggle REMOVED [supersedes the C-v2c toggle line; `showAllRemarks`/`toggleShowAllRemarks`/`hasRemarksAny` + controls-bar button deleted]; fixed-width panel inputs across all three edit blocks [numeric + text grid->flex-wrap `w-52`, Remarks `max-w-md`] so the Apply/Save button never needs horizontal scroll on a wide sheet; remark cap 500 -> 250 BOTH sides [`REMARK_MAX_LEN` + `_REMARK_MAX_LEN`] + the two cap tests retuned to 250/251; "Remarks: N" strip KEPT [independent `remarkCount`]; SheetReviewPage untouched; read path unchanged [a stored >250 remark still displays]; review_screen tests stay 95; tsc 0 wizard errors; Vite build exit 0 in container; NO doctype/schema change) // prior: 2026-06-07 (Slice C-v2c -- per-row Remarks; BACKEND new `remarks` Small Text field + NEW save_review_remark endpoint [set_value ONLY -- never edited_at/edit_log, row stays "Original"; blank clears to None; sheet_name VERBATIM] + remarks added to get_review_rows all_fields; FRONTEND blue MessageSquare marker inside the Classification cell [no new column] + detail-panel Textarea+counter+Save with DEDICATED remarkError + sheet-level "Remarks: N" strip; review_screen tests 90->95 +5) **Frontend conventions file: `frontend/CLAUDE.md` (NOT `frontend/.claude/CLAUDE.md`).**
...
|---|---|---|---|
| BoQ Upload & Management | `feature/boq-phase-3` | `frontend/.claude/plans/boq-upload-plan.md` | Phases 1.x (parser, 588 tests) + Phase 3 Module 1a/1b + Module 2a COMPLETE. Parse-run Phase 1 IN PROGRESS -- Bucket-2 COMPLETE (Slice 1 backend feat cb86b92b + Slice 2 frontend feat 21e56963). UNIFY-ON-(-1) + Slice A review-screen backend COMPLETE (feat fff26abd; has_human_parent retired; -1 sentinel convention; review_screen.py -- resolve_effective / check_structural_integrity / append_edit_log_entry + 3 endpoints; 34 new tests). Slice B1 review-screen spine COMPLETE (feat 0683f7b9; SheetReviewPage + ReviewTree + get_structural_breaks endpoint + 3 tests). Slice B1.1a COMPLETE (feat 58d2ed44; get_review_rows extended with column_descriptors; +13 review-screen tests). Slice B1.1b-i COMPLETE (feat 3e846ba1; FRONTEND -- descriptor-driven column layer + ClassificationPill + anchor letters; tsc 0 wizard errors; Vite build exit 0). Slice B1.1b-fix-A COMPLETE (BACKEND -- flatten_resolved_row writes human_parent=-1; agreement #54 unify completed; 60→63 parse-run tests). Slice B1.1b-fix-B COMPLETE (FRONTEND -- ReviewTree.tsx: parent column + scroll-to-parent, pill-no-clip, ancestor-only collapse, pill-above-text; tsc 0 wizard errors; Vite build exit 0). Slice B1.1b-ii COMPLETE (FRONTEND -- column-subset selector Popover + Checkbox + three independent spacer/note/subtotal visibility toggles; children-of-hidden-annotation render at original depth; tsc 0 wizard errors; Vite build exit 0). Slice B1.1b-iii COMPLETE (FRONTEND -- Description cell split: Classification fixed anchor [chevron+pill, flat-left] + text-only Description with depth indent; tsc 0 wizard errors; Vite build exit 0). Slice B2a COMPLETE (BACKEND + FRONTEND -- advisory flags: four §6.5.3 heuristics; get_structural_breaks extended to return flags; ReviewTree Info marker + click-to-reveal flag-reasons row; 76 review-screen tests, +26 from B2a; tsc 0 wizard errors; Vite build exit 0). Slice B2a single-source fix COMPLETE (refactor d9fa6b69 -- flags folded into get_review_rows; client-side computeAdvisoryFlags deleted ~87 lines; AdvisoryFlag exported from boqTypes; 77 review-screen tests). Slice B2a-fix COMPLETE (flag-(ii) rule tightened: fires only when amount_zero AND has_rate scalar -- qty-zero trigger dropped; new reason text; single-open accordion + master show/hide-all-notes toggle in ReviewTree; per-category flag count summary strip in SheetReviewPage; 78 review-screen tests +1; tsc 0 wizard errors; Vite build exit 0). Slice B2a-fix wording COMPLETE (FRONTEND text-only -- toggle "notes"→"flags", strip label "Advisory:"→"Flags:"; tsc 0 wizard errors; Vite build exit 0). Slice B2b COMPLETE (FRONTEND -- left-gutter expander column [frozen-left sticky], inline read-only detail panel [original-vs-effective, edit_log history, empty reason slot, provenance badge], Option-B mutual exclusivity with flag accordion, totalCols 5→6, sticky-header fix [solid bg on <th> cells, z-20/z-30], ClassificationPill restyle [rounded-full soft per-type fill, left-border dropped], aria-label "advisory notes"→"flags"; tsc 0 wizard errors; Vite build exit 0). Slice B2c COMPLETE (FRONTEND -- new "Status" fixed anchor column [after Excel Row, green "Edited" badge/blank on original, not frozen-left], green row tint on edited rows [bg-green-50 dark:bg-green-950/30, highlight flash still wins], provenance rule [edited_at non-null OR edit_log non-empty], totalCols 6→7, detail panel reshape [value-field block removed, edit-focused: provenance+orig-vs-eff+flags+edit_log+reason slot], panel badge amber→green; tsc 0 wizard errors; Vite build exit 0; read-only infra -- all rows ORIGINAL until Slice C). Slice B2c-fix COMPLETE (FRONTEND -- Status column unedited branch null→muted gray "Original" pill [bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300]; green "Edited" pill + row tint unchanged; tsc 0 wizard errors; Vite build exit 0). Slice C-v1 COMPLETE (feat 2bf77d62; BACKEND delta + read-side render -- first of three C-values sub-slices, the only backend-touching one; optional `reason` 6th edit_log key [uniform shape: always present, None when absent] via new trailing param on append_edit_log_entry + save_review_edit; `edited_at` added to save_review_edit return dict for C-v2's save-status anchor; EditLogEntry.reason?:string + ReviewTree edit-history per-entry reason render [row-level reason placeholder slot UNTOUCHED -- that is C-v2]; review_screen tests 78→84 +6; tsc 0 wizard errors; no Vite build [no behavioral UI change to live-cert]). Slice C-v2 COMPLETE (feat aa74a023; FRONTEND -- inline value-editing for the 7 flat numeric fields [qty_total/rate_supply/rate_install/rate_combined/amount_total/amount_supply/amount_install; mirrors backend _VALUE_FIELDS]; editable rule value_key===null && EDITABLE_VALUE_FIELDS.has(value_field); detail-panel Input+Apply per editable descriptor; single confirm AlertDialog with OPTIONAL reason; useFrappePostCall save_review_edit [sheet_name VERBATIM, reason raw->backend None]; onSaved+mutate refresh; sheet-level "All changes saved" anchor from returned edited_at; sub-line "Read-only review"->"Review & edit"; SaveReviewEditResponse type; per-area+text read-only [C-v2b]; tsc 0 wizard errors; Vite build exit 0 in container; live-cert is a separate next step). Slice C-v2b COMPLETE (feat ae65555c; BACKEND + FRONTEND -- inline text-editing for unit + make_model; backend _TEXT_FIELDS frozenset added to _ALLOWED_EDIT_FIELDS + new save_review_edit text write-branch [setattr verbatim, NO float() coercion; blank string clears to None; numeric float-path + human-field path unchanged]; text edit flips row to Edited via the same edited_at + edit_log path; FRONTEND EDITABLE_TEXT_FIELDS + editableTextDescriptors [same value_key===null gating as numeric] + separate "Edit text" detail-panel block [text Input + Apply] saving DIRECTLY [NO confirm dialog, silent save] via saveTextField reusing onSaved(edited_at); shared inline saveError below both edit blocks; description/row_notes stay read-only [explicitly excluded]; review_screen tests 84->90 +6 [2 verbatim-persist, blank-clear, description-rejected, row_notes-rejected, numeric-still-rejects-string]; tsc 0 wizard errors; Vite build exit 0 in container). Slice C-v2c COMPLETE (feat da6bb6d1; BACKEND + FRONTEND -- per-row Remarks [human-only annotation, NOT a data edit]; backend new `remarks` Small Text field on BoQ Review Row [migrated; column landed as PostgreSQL text] + NEW save_review_remark endpoint [writes ONLY remarks via frappe.db.set_value, NOT doc.save -- never stamps edited_at, never appends edit_log, so the row stays "Original"; 250-char hard guard via frappe.throw [was 500, lowered in C-v2c-polish]; blank/whitespace clears to None; sheet_name VERBATIM #152] + remarks added to get_review_rows all_fields [explicit named list]; save_review_edit UNTOUCHED; FRONTEND ReviewRow.remarks type + separate save_review_remark POST [own loading flag] + blue MessageSquare marker in a right-aligned marker-group wrapper INSIDE the Classification cell next to the flag marker [NO new column, totalCols unchanged] gated hasRemark && (showAllRemarks || expandedDetailRow===idx) [mirrors flags marker, reveal=panel not a row] + master Show/Hide-all-remarks toggle [default off] + detail-panel "Remarks" block [Textarea + live 500-counter + Save disabled over-cap/unchanged/saving] with DEDICATED remarkError [separate from shared saveError] + onRemarkSaved prop [mutate WITHOUT advancing the edit anchor] + sheet-level "Remarks: N" strip mirroring the flags strip; review_screen tests 90->95 +5 [persist-no-provenance, blank-clear, exactly-500, 501-rejected, value-edit-still-stamps]; tsc 0 errors; Vite build exit 0 in container). Slice C-v2c-polish COMPLETE (fix ae9dcff2; FRONTEND + cap -- remark marker ALWAYS-VISIBLE [gate simplified to just hasRemark]; master Show/Hide-all-remarks toggle REMOVED [supersedes the C-v2c toggle line; showAllRemarks/toggleShowAllRemarks/hasRemarksAny + controls-bar button deleted]; fixed-width panel inputs across all three edit blocks [numeric + text grid->flex-wrap w-52, Remarks max-w-md] so the Apply/Save button never needs horizontal scroll on a wide sheet; remark cap 500->250 BOTH sides [REMARK_MAX_LEN + _REMARK_MAX_LEN] + the two cap tests retuned to 250/251; "Remarks: N" strip KEPT [independent remarkCount]; SheetReviewPage untouched; read path unchanged [a stored >250 remark still displays]; review_screen tests stay 95; tsc 0 wizard errors; Vite build exit 0 in container; NO doctype/schema change). Slice C-v2d COMPLETE (feat cd2cc156; BACKEND + FRONTEND -- per-area numeric editing [qty/amount/rate] via an EXTENDED save_review_edit [new optional area + rate_subkey params; area=None == prior flat path]; FLAT one-hop qty_by_area/amount_by_area + NESTED two-hop rate_by_area [rate_subkey from _LEGAL_RATE_SUBKEYS = frozenset(_RATE_ROLE_TO_KIND.values()), REUSED from parser not copied]; bare-dict read-modify-write riding the existing _RESAVE_LIST_JSON_FIELDS list-sibling pre-serialization [the REASON it EXTENDS the endpoint, not a fresh path]; blank -> 0.0 with the area key KEPT [never deleted]; STRUCTURED edit_log keying [optional area+rate_subkey, old flat entries omit them and stay valid]; validation rejects nonexistent-area / missing-rate_subkey / illegal-rate_subkey / non-per-area-field-with-area; append_edit_log_entry + return shape gain area/rate_subkey; FRONTEND ReviewTree "Edit per-area values" block mirroring the C-v2 confirm-dialog pattern [openAreaConfirm -> same AlertDialog -> confirmValueSave sends area+rate_subkey; row flips to "Edited"] + rate_by_area TS type fix flat->nested [RateByAreaCell; no static read site broke] + EditLogEntry/SaveReviewEditResponse area/rate_subkey + Obs A edit_log render LATEST-FIRST; review_screen tests 95->105 +10 [TestSaveReviewEditPerArea]; tsc 0 wizard errors in-container; Vite build exit 0 in-container; NO doctype/schema change; LIVE-CERT NOT YET DONE -- rate-editing live-cert OWED against a Pattern-2-rate vehicle [Electrical has no nested rate data], BOQ-26-00145 doubly-dirtied by probes [re-parse before live-cert]). Wizard tests: 276 total DERIVED (test_parse_run 63 + test_review_screen 105 [measured this run] + test_update_sheet_draft 66 + test_update_boq_draft 7 + test_sheet_preview 23 + test_upload_file 12; full suite NOT re-run this session -- only test_review_screen ran); frontend slices verified by tsc + Vite build. Full sub-phase history + as-built detail: see boq-upload-plan.md (per-slice sections). Do not duplicate the changelog here. |
...
- `run_parse(boq_name, sheet_names=None)` -- `@frappe.whitelist(methods=["POST"])`. Enqueues `_run_parse_worker` on `queue="long"`, `timeout=600`. `sheet_names=None` parses all eligible Reviewed/Parsed sheets; `sheet_names=[...]` parses only the named subset (per-sheet re-parse). After successful enqueue sets `BOQs.parse_in_progress=1` + commits (Bucket-2 Slice 1, feat cb86b92b). Returns `{"status":"queued","job_id":...}`. URL: `/api/method/nirmaan_stack.api.boq.wizard.parse_run.run_parse`
...
- Review edits (human layer, Slice A): `human_classification` (Data), `human_parent` (Int), `edit_log` (JSON list), `edited_by` (Data), `edited_at` (Datetime). `has_human_parent` DOES NOT EXIST (was retired in feat fff26abd -- see -1 sentinel below). `remarks` (Small Text, Slice C-v2c) also lives in this section but is NOT an edit field -- it is human-only annotation written ONLY by `save_review_remark`, never stamps edited_at/edit_log, and a remark-only row stays "Original".
...
- `mark_sheet_parsed_check_done(boq_name, sheet_name, confirm=False)` -- `@frappe.whitelist(methods=["POST"])`. Step 1: run check_structural_integrity. Step 2a: breaks exist + confirm=False -> return `{ok:False, breaks:[...]}` (warn-and-confirm gate). Step 2b: clean or confirm=True -> set wizard_status="Parsed Check Done" directly (bypasses set_sheet_status). Returns `{ok, status, overridden}`. URL: `/api/method/nirmaan_stack.api.boq.wizard.review_screen.mark_sheet_parsed_check_done`
