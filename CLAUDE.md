...
**Last updated:** 2026-06-07 (Slice C-v2 -- inline value-editing for the 7 flat numeric fields; FRONTEND-only on C-v1's backend; first human edit written through the UI; tsc 0 wizard errors; Vite build exit 0 in container; wizard tests unchanged) **Frontend conventions file: `frontend/CLAUDE.md` (NOT `frontend/.claude/CLAUDE.md`).**
...
- `save_review_edit(boq_name, sheet_name, row_index, field, value, reason=None)` -- `@frappe.whitelist(methods=["POST"])`. Editable fields: human_classification, human_parent, qty_total, rate_supply, rate_install, rate_combined, amount_total, amount_supply, amount_install. Cycle-guard + self-parent guard for human_parent. Appends to edit_log; stamps edited_by + edited_at. Pre-serializes _RESAVE_LIST_JSON_FIELDS before doc.save() (Defect-1 fix). `reason` (Slice C-v1) is an OPTIONAL free-text note (blank/whitespace normalized to None) stored as the 6th key on the edit_log entry -- entry shape now `{field, from, to, by, at, reason}`; no UI writer yet (C-v2 adds the input). Returns `{ok, row_index, field, from, to, edited_at, effective}` (edited_at added Slice C-v1). URL: `/api/method/nirmaan_stack.api.boq.wizard.review_screen.save_review_edit`
