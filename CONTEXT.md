...
- **Raiser's own step** — an approval step the person raising a payment or expense already holds is not asked of them again. An **L1 approver** (the Admin profile, or `Administrator`) raising one between ₹15,000 and ₹50,000 lands it at *Approved*; above ₹50,000 it lands at *CEO Pending* with L1 already done. The **CEO** raising one lands it at *Approved* at any amount. Everyone else follows the plain amount rule. For an Expense the raiser is whoever raised its **Expense Request**, never the reviewer who approved that request. Accountants are not L1 for this rule. Applies to PO payments, Work Order payments, Project Expenses and Non-Project Expenses. (Owner, 2026-09-21.)

- **Revert to Approved** — sending a Project Payment at *Reconciliation Pending* back to *Approved*, meaning "it was marked done by mistake". From *Approved* it can be marked done again or deleted. It is refused while a bank line is matched to the payment. Admin, Accountant and Accountant Lead only. (Owner, 2026-09-21.)

- **Editing a record by status** — a PO or Work Order payment cannot be edited in the app at any status; the only correction is *Revert to Approved*. An Expense can be edited at every status except *Rejected*; once *Paid*, its amount, payment date and payment reference can no longer be changed in the app. **These are screen rules only**: the server locks nothing on a *Paid* record, so a correction can still be made in Desk. Editing a *Paid* payment's amount there does not update the PO / Work Order's paid total. (Owner, 2026-09-21.)
