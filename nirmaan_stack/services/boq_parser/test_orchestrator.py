...
# ================================================================ #
# Bug 11 (sec 9 #87) — pattern-consistency real-fixture tests       #
# ================================================================ #

_BOQ_ELEC_FIXTURE = _FIXTURES / "Bill of Quantities.xlsx"
_BOQ_ELEC_SHEET = "ELECTRICAL & ELV BOQ"
_BOQ_ELEC_HEADER_ROW = 2

_V2_FIXTURE = _FIXTURES / "multi_area_single_header_v2.xlsx"
_V2_HVAC_SHEET = "HVAC BOQ"
_V2_HEADER_ROW = 2


class TestBug11BoqElecIntegration(unittest.TestCase):
    """
    Bug 11 (sec 9 #87) — real-fixture integration test against
    'Bill of Quantities.xlsx', 'ELECTRICAL & ELV BOQ' sheet.

    Fixture structure (auto-guessed at hrc=2, header_row=2):
      A=sl_no, B=append_to_notes, C=description, D=unit, E=qty

    sl_no values like '1.0', '2.0', ... '10.0' (signature 'D.D', depth=2)
    appear as both PREAMBLE (section header, no qty) and LINE_ITEM (with qty).
    Bug 11 post-pass flags all such rows with review_reason=
    'pattern_consistency_mismatch_depth_2' but leaves classifications unchanged
    (depth=2, flag-only policy).

    Per Bug 11 constraint: classifier-level assertions only (no hierarchy fields).
    DISCLOSED: empirical run shows 417 flagged rows (threshold >= 400 used).
    """
    ...
    def setUpClass(cls):
        ...
        cls._RC = RowClassification
        if not _BOQ_ELEC_FIXTURE.exists():
            return
        kws = GlobalSettings().multi_area_reserved_keywords
        reader = BoqReader(str(_BOQ_ELEC_FIXTURE))
        sc = auto_guess_sheet_config(
            reader, _BOQ_ELEC_SHEET, _BOQ_ELEC_HEADER_ROW,
            header_row_count=2, reserved_keywords=kws,
        )
        mc = MappingConfig(
            project="bug11_boq_elec_test",
            master_boq=MasterBoqMetadata(boq_name="Bug 11 BOQ Electrical"),
            sheets=[sc],
        )
        result = parse_boq(str(_BOQ_ELEC_FIXTURE), mc)
        elec = next(
            (s for s in result.sheets if s.sheet_name == _BOQ_ELEC_SHEET), None
        )
        cls.resolved = elec.resolved_rows if elec else []

    def test_fixture_exists(self):
        self.assertTrue(
            _BOQ_ELEC_FIXTURE.exists(),
            f"Fixture missing: {_BOQ_ELEC_FIXTURE}",
        )

    def test_row_1_0_preamble_flagged(self):
        """
        Excel row 6 (sl_no='1.0'): PREAMBLE in a 'D.D' mixed group.
        Bug 11 must set needs_classification_review=True and
        review_reason='pattern_consistency_mismatch_depth_2'.
        """
        matches = [
            rr for rr in self.resolved
            if rr.classified_row.sl_no_value == "1.0"
            and rr.classified_row.classification == self._RC.PREAMBLE
        ]
        self.assertGreaterEqual(len(matches), 1, "Expected a PREAMBLE row with sl_no='1.0'")
        row = matches[0]
        self.assertTrue(
            row.classified_row.needs_classification_review,
            "sl_no='1.0' PREAMBLE must be flagged by Bug 11",
        )
        self.assertEqual(
            row.classified_row.review_reason,
            "pattern_consistency_mismatch_depth_2",
        )

    def test_row_10_0_preamble_flagged(self):
        """
        Excel row 38 (sl_no='10.0'): PREAMBLE in 'D.D' mixed group.
        Must carry depth_2 review flag.
        """
        matches = [
            rr for rr in self.resolved
            if rr.classified_row.sl_no_value == "10.0"
            and rr.classified_row.classification == self._RC.PREAMBLE
        ]
        self.assertGreaterEqual(len(matches), 1, "Expected a PREAMBLE row with sl_no='10.0'")
        row = matches[0]
        self.assertTrue(row.classified_row.needs_classification_review)
        self.assertEqual(
            row.classified_row.review_reason,
            "pattern_consistency_mismatch_depth_2",
        )

    def test_total_depth2_flagged_rows_threshold(self):
        """
        Empirical: 417 rows flagged with depth_2 review reason.
        Threshold >= 400 is safely below 417 and above pre-fix 0.
        """
        flagged = [
            rr for rr in self.resolved
            if rr.classified_row.review_reason == "pattern_consistency_mismatch_depth_2"
        ]
        self.assertGreaterEqual(
            len(flagged), 400,
            f"Expected >= 400 depth_2 flagged rows, got {len(flagged)}",
        )

    def test_zero_depth1_flagged_rows(self):
        """
        No depth-1 mixed groups exist in this workbook.
        Zero rows must carry review_reason='pattern_consistency_mismatch_depth_1'.
        """
        flagged_depth1 = [
            rr for rr in self.resolved
            if rr.classified_row.review_reason == "pattern_consistency_mismatch_depth_1"
        ]
        self.assertEqual(
            len(flagged_depth1), 0,
            f"Expected 0 depth_1 flagged rows, got {len(flagged_depth1)}",
        )


class TestBug11V2HvacIntegration(unittest.TestCase):
    """
    Bug 11 (sec 9 #87) — real-fixture integration test against
    'multi_area_single_header_v2.xlsx', 'HVAC BOQ' sheet.

    Fixture structure (auto-guessed at hrc=1, header_row=2):
      sl_no='D.D' pattern (e.g. '2.1') appears as both PREAMBLE and LINE_ITEM.

    DISCLOSED DEVIATION from prompt: prompt assumed v2 would contain depth-1
    auto-promote cases ('D, D+, D0, A.' shapes). Empirical parse found only
    depth-2 mixed groups (sig 'D.D' in HVAC BOQ) — no depth-1 mixed groups.
    These tests are therefore depth-2 flag-only assertions, not depth-1 promote
    assertions. This deviation is recorded in the feat commit message.

    Per Bug 11 constraint: classifier-level assertions only (no hierarchy fields).
    DISCLOSED: empirical run shows ~15 depth-2 flagged rows (threshold >= 5).
    """
    ...
    def setUpClass(cls):
        ...
        cls._RC = RowClassification
        if not _V2_FIXTURE.exists():
            return
        kws = GlobalSettings().multi_area_reserved_keywords
        reader = BoqReader(str(_V2_FIXTURE))
        sc = auto_guess_sheet_config(
            reader, _V2_HVAC_SHEET, _V2_HEADER_ROW,
            header_row_count=1, reserved_keywords=kws,
        )
        mc = MappingConfig(
            project="bug11_v2_hvac_test",
            master_boq=MasterBoqMetadata(boq_name="Bug 11 V2 HVAC"),
            sheets=[sc],
        )
        result = parse_boq(str(_V2_FIXTURE), mc)
        hvac = next(
            (s for s in result.sheets if s.sheet_name == _V2_HVAC_SHEET), None
        )
        cls.resolved = hvac.resolved_rows if hvac else []

    def test_fixture_exists(self):
        self.assertTrue(
            _V2_FIXTURE.exists(),
            f"Fixture missing: {_V2_FIXTURE}",
        )

    def test_row_2_1_preamble_flagged(self):
        """
        Excel row 23 (sl_no='2.1'): PREAMBLE in 'D.D' mixed group.
        Bug 11 must flag it with review_reason='pattern_consistency_mismatch_depth_2'.
        """
        matches = [
            rr for rr in self.resolved
            if rr.classified_row.sl_no_value == "2.1"
            and rr.classified_row.classification == self._RC.PREAMBLE
        ]
        self.assertGreaterEqual(
            len(matches), 1,
            "Expected a PREAMBLE row with sl_no='2.1' in HVAC BOQ",
        )
        row = matches[0]
        self.assertTrue(
            row.classified_row.needs_classification_review,
            "sl_no='2.1' PREAMBLE must be flagged by Bug 11",
        )
        self.assertEqual(
            row.classified_row.review_reason,
            "pattern_consistency_mismatch_depth_2",
    ...
    def test_some_depth2_flagged_rows_present(self):
        """
        Empirical: ~15 rows flagged with depth_2 review reason in HVAC BOQ.
        Threshold >= 5 is conservative.
        """
        flagged = [
            rr for rr in self.resolved
            if rr.classified_row.review_reason == "pattern_consistency_mismatch_depth_2"
        ...
        self.assertGreaterEqual(
            len(flagged), 5,
            f"Expected >= 5 depth_2 flagged rows in HVAC BOQ, got {len(flagged)}",
        )

    def test_zero_depth1_flagged_rows(self):
        """
        No depth-1 mixed groups exist in this sheet.
        Zero rows must carry review_reason='pattern_consistency_mismatch_depth_1'.
        """
        flagged_depth1 = [
            rr for rr in self.resolved
    ...
    def test_no_depth1_promotions_occurred(self):
        """
        Since there are no depth-1 mixed groups, no LINE_ITEM should have been
        promoted to PREAMBLE by Bug 11 in this sheet.
        All PREAMBLE rows must have been PREAMBLE prior to the post-pass
        (verified indirectly: no depth-1 flag present).
        This test is DISCLOSED to differ from the prompt's assumption of
        depth-1 auto-promote cases in v2.
        """
        depth1_flagged = [
            rr for rr in self.resolved
            if rr.classified_row.review_reason == "pattern_consistency_mismatch_depth_1"
        ]
        self.assertEqual(len(depth1_flagged), 0)
