...
from nirmaan_stack.services.boq_parser.classifier import (
    ...
    _compute_pattern_depth,
    _compute_pattern_signature,
    _is_cross_row_sum,
    classify_row,
)
...
# ================================================================ #
# Bug 11 (sec 9 #87) — pattern-consistency post-pass tests         #
# ================================================================ #
...
def _make_classified(
    ...
    """Minimal ClassifiedRow for post-pass unit tests."""
    return ClassifiedRow(
...
class TestBug11PatternSignatureHelper(unittest.TestCase):
    """
    Unit tests for _compute_pattern_signature().

    DISCLOSED DEVIATION from spec: spec says 'letters replaced with L'
    (case-folded). Actual implementation is case-PRESERVING:
      uppercase letters → 'U', lowercase letters → 'l'.
    This prevents false groupings of e.g. 'A.' (PREAMBLE header) and 'a.'
    (LINE_ITEM sub-item) in snitch_electrical.xlsx — mixing those groups
    would force §7.29 qty→0 on the LINE_ITEMs, breaking the fixture.
    """

    def test_empty_string(self):
        self.assertEqual(_compute_pattern_signature(""), "")

    def test_single_digit(self):
        self.assertEqual(_compute_pattern_signature("1"), "D")

    def test_digit_dot_digits(self):
        self.assertEqual(_compute_pattern_signature("1.03"), "D.DD")

    def test_three_part_numeric(self):
        self.assertEqual(_compute_pattern_signature("1.1.5"), "D.D.D")

    def test_uppercase_letter(self):
        # spec says 'L'; actual: uppercase → 'U'  (case-preserving deviation)
        self.assertEqual(_compute_pattern_signature("A"), "U")

    def test_lowercase_letter(self):
        # spec says 'L'; actual: lowercase → 'l'  (case-preserving deviation)
        self.assertEqual(_compute_pattern_signature("a"), "l")

    def test_uppercase_dot_digit(self):
        # "D.0" — 'D' is uppercase → 'U'; '0' is digit → 'D'
        self.assertEqual(_compute_pattern_signature("D.0"), "U.D")

    def test_uppercase_plus_dot_digit(self):
        # "D+.0" — 'D'→'U', '+'→'+', '.'→'.', '0'→'D'
        self.assertEqual(_compute_pattern_signature("D+.0"), "U+.D")

    def test_mixed_upper_digit(self):
        # "D0.0" — 'D'→'U', '0'→'D', '.'→'.', '0'→'D'
        self.assertEqual(_compute_pattern_signature("D0.0"), "UD.D")

    def test_digit_lowercase(self):
        # "1a" — '1'→'D', 'a'→'l'
        self.assertEqual(_compute_pattern_signature("1a"), "Dl")
    ...
    def test_uppercase_dot(self):
        # "A." — 'A'→'U', '.'→'.' — distinct from "a." → "l."
        self.assertEqual(_compute_pattern_signature("A."), "U.")


class TestBug11PatternDepthHelper(unittest.TestCase):
    """Unit tests for _compute_pattern_depth()."""

    def test_empty_string(self):
        self.assertEqual(_compute_pattern_depth(""), 0)

    def test_single_token(self):
        self.assertEqual(_compute_pattern_depth("1"), 1)

    def test_two_tokens(self):
        self.assertEqual(_compute_pattern_depth("1.03"), 2)

    def test_three_tokens(self):
        self.assertEqual(_compute_pattern_depth("1.1.5"), 3)

    def test_trailing_dot(self):
        # "1." splits to ["1", ""] — empty token excluded → depth 1
        self.assertEqual(_compute_pattern_depth("1."), 1)

    def test_leading_dot(self):
        # ".5" splits to ["", "5"] — empty token excluded → depth 1
        self.assertEqual(_compute_pattern_depth(".5"), 1)

    def test_double_dot(self):
        # "1..2" splits to ["1", "", "2"] — empty excluded → depth 2
        self.assertEqual(_compute_pattern_depth("1..2"), 2)

    def test_letter_token(self):
        # "A" → depth 1
        self.assertEqual(_compute_pattern_depth("A"), 1)

    def test_letter_dot_digit(self):
        # "A.1" → depth 2
        self.assertEqual(_compute_pattern_depth("A.1"), 2)


class TestBug11PatternConsistencyPostPass(unittest.TestCase):
    """
    Unit tests for _apply_pattern_consistency_post_pass().

    Tests use hand-constructed ClassifiedRow lists; no xlsx fixtures needed.
    """

    _P = RowClassification.PREAMBLE
    _L = RowClassification.LINE_ITEM
    _N = RowClassification.NOTE

    def test_pure_preamble_group_unchanged(self):
        """All rows in a group already PREAMBLE → no flags, no promotion."""
        rows = [
            _make_classified(1, "1.0", self._P),
            _make_classified(2, "1.0", self._P),
        ]
        _apply_pattern_consistency_post_pass(rows)
        for r in rows:
            self.assertFalse(r.needs_classification_review)
            self.assertEqual(r.review_reason, "")

    def test_pure_line_item_group_unchanged(self):
        """All rows in a group already LINE_ITEM → no flags."""
        rows = [
            _make_classified(1, "1.1", self._L),
            _make_classified(2, "1.1", self._L),
        ]
        _apply_pattern_consistency_post_pass(rows)
        for r in rows:
            self.assertFalse(r.needs_classification_review)

    def test_depth_1_mixed_flags_both_no_promotion(self):
        """
        depth=1 mixed group: flag-only (no auto-promotion per DISCLOSED deviation).
        Both rows flagged with review_reason='pattern_consistency_mismatch_depth_1'.
        Classifications unchanged — LINE_ITEM stays LINE_ITEM.

        DISCLOSED DEVIATION: spec prescribed auto-promotion at depth<=1; empirical
        snitch_electrical regression (95 LINE_ITEMS) mandates flag-only instead.
        """
        rows = [
            _make_classified(1, "1", self._P),
            _make_classified(2, "1", self._L),
        ]
        _apply_pattern_consistency_post_pass(rows)
        self.assertEqual(rows[0].classification, self._P)   # unchanged
        self.assertEqual(rows[1].classification, self._L)   # NOT promoted (flag-only)
        for r in rows:
            self.assertTrue(r.needs_classification_review)
            self.assertEqual(r.review_reason, "pattern_consistency_mismatch_depth_1")

    def test_depth_2_mixed_no_promotion(self):
        """
        depth=2 mixed group: classifications unchanged; both rows flagged
        with review_reason='pattern_consistency_mismatch_depth_2'.
        """
        rows = [
            _make_classified(1, "1.0", self._P),
            _make_classified(2, "1.0", self._L),
        ]
        _apply_pattern_consistency_post_pass(rows)
        self.assertEqual(rows[0].classification, self._P)
        self.assertEqual(rows[1].classification, self._L)  # NOT promoted
        for r in rows:
            self.assertTrue(r.needs_classification_review)
            self.assertEqual(r.review_reason, "pattern_consistency_mismatch_depth_2")

    def test_depth_3_mixed_no_action(self):
        """depth >= 3 mixed group: no flags, no promotion."""
        rows = [
            _make_classified(1, "1.1.1", self._P),
            _make_classified(2, "1.1.1", self._L),
        ]
        _apply_pattern_consistency_post_pass(rows)
        self.assertEqual(rows[0].classification, self._P)
        self.assertEqual(rows[1].classification, self._L)
        for r in rows:
            self.assertFalse(r.needs_classification_review)

    def test_non_preamble_line_item_rows_excluded(self):
        """NOTE rows are excluded from grouping — no flags applied."""
        rows = [
            _make_classified(1, "1", self._N),
            _make_classified(2, "1", self._L),
        ]
        _apply_pattern_consistency_post_pass(rows)
        # Only 1 row in the group (the LINE_ITEM) → not a mixed group
        self.assertFalse(rows[0].needs_classification_review)
        self.assertFalse(rows[1].needs_classification_review)

    def test_empty_sl_no_excluded_from_grouping(self):
        """Rows with empty sl_no_value are excluded from sig grouping."""
        rows = [
            _make_classified(1, "", self._P),
            _make_classified(2, "", self._L),
        ]
        _apply_pattern_consistency_post_pass(rows)
        for r in rows:
            self.assertFalse(r.needs_classification_review)

    def test_distinct_signatures_in_same_list(self):
        """
        Two separate signature groups in one list; only the mixed one gets flagged.
        sig 'D.DD' (depth=2, mixed) → flag; sig 'D.D' (depth=2, pure PREAMBLE) → no flag.
        """
        rows = [
            _make_classified(1, "1.00", self._P),   # sig D.DD — mixed
            _make_classified(2, "1.00", self._L),   # sig D.DD — mixed
            _make_classified(3, "2.0", self._P),    # sig D.D — pure
            _make_classified(4, "2.0", self._P),    # sig D.D — pure
        ]
        _apply_pattern_consistency_post_pass(rows)
        self.assertTrue(rows[0].needs_classification_review)
        self.assertTrue(rows[1].needs_classification_review)
        self.assertFalse(rows[2].needs_classification_review)
        self.assertFalse(rows[3].needs_classification_review)

    def test_review_reason_not_overwritten_when_already_set(self):
        """
        If a row already has review_reason set (e.g. from a prior pass),
        the post-pass must NOT overwrite it.
        """
        row_p = _make_classified(1, "1", self._P)
        row_l = _make_classified(2, "1", self._L)
        row_p.needs_classification_review = True
        row_p.review_reason = "priced_preamble_with_children"
        _apply_pattern_consistency_post_pass([row_p, row_l])
        # row_p already had a reason — must not be overwritten
        self.assertEqual(row_p.review_reason, "priced_preamble_with_children")
        # row_l had no prior reason — must be flagged
        self.assertEqual(row_l.review_reason, "pattern_consistency_mismatch_depth_1")

    def test_case_preserving_uppercase_lowercase_different_groups(self):
        """
        Uppercase 'A.' (sig 'U.') and lowercase 'a.' (sig 'l.') must NOT be
        grouped together. Case-preserving normalization prevents false promotions
        that would break snitch_electrical fixture.
        """
        rows = [
            _make_classified(1, "A.", self._P),    # sig 'U.' — pure PREAMBLE
            _make_classified(2, "a.", self._L),    # sig 'l.' — pure LINE_ITEM
        ]
        _apply_pattern_consistency_post_pass(rows)
        # Different sigs → no mixing → no flags
        self.assertEqual(rows[0].classification, self._P)
        self.assertEqual(rows[1].classification, self._L)
        self.assertFalse(rows[0].needs_classification_review)
        self.assertFalse(rows[1].needs_classification_review)

    def test_depth_1_multiple_mixed_all_flagged_not_promoted(self):
        """All rows in a depth-1 mixed group are flagged; LINE_ITEMs stay LINE_ITEM."""
        rows = [
            _make_classified(1, "A", self._P),
            _make_classified(2, "A", self._L),
            _make_classified(3, "A", self._L),
        ]
        _apply_pattern_consistency_post_pass(rows)
        self.assertEqual(rows[0].classification, self._P)
        self.assertEqual(rows[1].classification, self._L)  # NOT promoted
        self.assertEqual(rows[2].classification, self._L)  # NOT promoted
        for r in rows:
            self.assertTrue(r.needs_classification_review)
            self.assertEqual(r.review_reason, "pattern_consistency_mismatch_depth_1")

    def test_whitespace_sl_no_excluded(self):
        """sl_no containing only whitespace is treated as empty — excluded."""
        rows = [
            _make_classified(1, "   ", self._P),
            _make_classified(2, "   ", self._L),
        ]
...
class TestBug11OrchestratorIntegration(unittest.TestCase):
    """
    End-to-end tests: ClassifiedRows → _apply_pattern_consistency_post_pass()
    → resolve_hierarchy() → ResolvedRow.needs_classification_review / review_reason.

    No xlsx fixture required — ClassifiedRows are hand-constructed.
    Per the Bug 11 constraint: NO hierarchy-level assertions (parent_index, level,
    path). Only classifier-level outcomes verified (flag + review_reason).
    Hierarchy assertions are deferred to Bug 12.
    """
    ...
    def _resolve(self, rows: list[ClassifiedRow]) -> list:
        from nirmaan_stack.services.boq_parser.hierarchy import resolve_hierarchy
        sc = self._build_config()
        sheet = resolve_hierarchy(rows, sc, _GS)
        return sheet.rows

    def test_depth_2_flags_carried_to_resolved_rows(self):
        """
        depth=2 mixed group: flags set by post-pass must survive resolve_hierarchy()
        and appear on the corresponding ResolvedRows.
        """
        rows = [
            _make_classified(1, "1.0", RowClassification.PREAMBLE),
            _make_classified(2, "1.0", RowClassification.LINE_ITEM, qty=5.0),
        ]
        _apply_pattern_consistency_post_pass(rows)
        resolved = self._resolve(rows)
        flagged = [r for r in resolved if r.needs_classification_review]
        self.assertEqual(len(flagged), 2, "Both rows in the group must be flagged")
        for r in flagged:
            self.assertEqual(r.review_reason, "pattern_consistency_mismatch_depth_2")

    def test_depth_1_flags_carried_to_resolved_rows(self):
        """
        depth=1 mixed group (flag-only): both rows are flagged in the resolved
        output. Classifications unchanged — LINE_ITEM stays LINE_ITEM.

        DISCLOSED DEVIATION from spec: no auto-promotion; see post-pass docstring.
        """
        rows = [
            _make_classified(1, "A", RowClassification.PREAMBLE),
            _make_classified(2, "A", RowClassification.LINE_ITEM, qty=3.0),
        ]
        _apply_pattern_consistency_post_pass(rows)
        resolved = self._resolve(rows)
        # Both rows must be flagged
        flagged = [r for r in resolved if r.needs_classification_review]
        self.assertEqual(len(flagged), 2, "Both rows in depth-1 group must be flagged")
        for r in flagged:
            self.assertEqual(r.review_reason, "pattern_consistency_mismatch_depth_1")
        # LINE_ITEM stays LINE_ITEM (no promotion)
        line_items = [
            r for r in resolved
            if r.classified_row.classification == RowClassification.LINE_ITEM
        ]
        self.assertEqual(len(line_items), 1, "LINE_ITEM must not be promoted")

    def test_unflagged_rows_remain_clean_in_resolved(self):
        """
        Rows in pure (non-mixed) groups must not carry flags after resolve_hierarchy().
        Use distinct signatures so each group has exactly one classification:
          - sl_no="A.1" (sig "U.D", depth=2) → PREAMBLE singleton → pure → no flag
          - sl_no="1.2.3" (sig "D.D.D", depth=3) → LINE_ITEM → excluded (depth>=3) → no flag
        """
        rows = [
            _make_classified(1, "A.1", RowClassification.PREAMBLE),
            _make_classified(2, "1.2.3", RowClassification.LINE_ITEM, qty=7.0),
        ]
        _apply_pattern_consistency_post_pass(rows)
        resolved = self._resolve(rows)
        for r in resolved:
