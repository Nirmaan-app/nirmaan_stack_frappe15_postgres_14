...
class TestAllocatedOf(unittest.TestCase):
    ...
    def test_a_negative_leg_counts_as_the_money_it_moved(self):
        """THE OLD CREDIT PATH. The removed B7 `inflows.create_non_project_receipt` recorded a bank
        CREDIT as a NEGATIVE `Non Project Expense`, and the legs it wrote may still exist (#1266
        keeps this read side), so such a leg's `target_amount` is below zero. The ROW's amount is a
    ...
    def test_a_recorded_receipt_leaves_its_row_fully_allocated(self):
        ...
        stuck at `Partially Allocated` for good."""
        legs = [_leg("-5000")]
