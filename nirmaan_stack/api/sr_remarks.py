...
from nirmaan_stack.services.role_profiles import SERVICE_PROCUREMENT_PROFILES

# Role to subject mapping. Every procurement profile files under the same
# subject, so this is built from SERVICE_PROCUREMENT_PROFILES rather than listing them.
ROLE_SUBJECT_MAP = {
    "Nirmaan Accountant Profile": "accountant_remark",
    "Nirmaan Accountant Lead Profile": "accountant_remark",
    **{profile: "procurement_remark" for profile in SERVICE_PROCUREMENT_PROFILES},
...
@frappe.whitelist()
def add_sr_remark(sr_id: str, content: str) -> dict:
    """
    Add a remark to a Service Request.
...
@frappe.whitelist()
def get_sr_remarks(sr_id: str, subject_filter: str = None) -> dict:
    """
...
@frappe.whitelist()
def get_sr_remarks_count(sr_id: str) -> dict:
    """
    Get the count of remarks for a Service Request.
...
@frappe.whitelist()
def delete_sr_remark(remark_id: str) -> dict:
    """
...
@frappe.whitelist()
def get_sr_recent_remarks(sr_id: str, limit: int = 3) -> dict:
    """
    Get recent remarks for a SR (for hover display in tables).
