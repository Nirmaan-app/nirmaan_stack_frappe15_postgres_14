...
def current_sheet_pricing_any_version(
    boq_name: str, sheet_name: str, current_version=None
) -> list:
    """The CURRENT pricing records for one (boq, sheet) across EVERY committed version
    (ADR-0014 A10 / Amendment B W6). NOT whitelisted -- an internal cross-BOQ carry reader.

    Why this exists: pricing identity includes `committed_version` (`_IDENTITY_FIELDS`), so
    `is_current` is scoped PER VERSION. Re-committing a sheet mints a new committed version and
    ORPHANS the prior version's pricing onto the now-frozen version (`commit_pipeline` warns via
    the BOQ_DOWNSTREAM_ORPHAN guard but never migrates it). A strictly version-pinned read --
    `get_sheet_pricing(committed_version=...)` -- therefore returns NOTHING for a sheet whose
    rates were entered before its last re-commit, which is exactly the defect this fixes: the
    revision mapping screen promised "3 rates" and the carry landed zero.

    Dedup rule: a cell CAN hold one current row per committed version, so `(excel_row,
    col_letter)` is deduped by preferring, in order:
    ...
    Anchoring to the committed SHEET rather than to `MAX(committed_version)` matters because the
    two are not the same question. A bare max assumes the largest version number is the live one;
    it is not authoritative, and a pricing row stranded ABOVE the current sheet version (a rolled
    back or otherwise superseded commit) would then beat the price the user can actually see on
    the current sheet. Rule 1 makes the visible price win by construction. `current_version` is
    resolved here when the caller does not pass it; None (no current committed sheet) degrades to
    rules 2-3.

    Deliberate asymmetry, do NOT "fix" into symmetry: RATES read cross-version (here); STRUCTURE
    (nodes / descriptions / the row match) stays pinned per-commit. Pricing is a living layer that
    keeps being edited after the structure freezes.

    Returns a plain list of the same field dicts `get_sheet_pricing` returns, ordered
    `excel_row asc, col_letter asc`. sheet_name matched VERBATIM (#152).
    """
    ...
    rows = frappe.get_all(
        _PRICING,
        filters={"boq": boq_name, "sheet_name": sheet_name, "is_current": 1},
        fields=[
            "name", "boq", "sheet_name", "excel_row", "col_letter", "committed_version",
            "node", "area", "rate_kind", "description",
            "rate", "is_filled", "pricing_version", "is_current", "priced_at", "is_finalized",
    ...
    def _rank(r):
        # Tuples compare elementwise and True > False, so a row on the current committed version
        # outranks every other version regardless of how high that version's number is.
        return (
            current_version is not None and r["committed_version"] == current_version,
            r["committed_version"] or 0,
            r["pricing_version"] or 0,
        )

    latest: dict = {}
    for r in rows:
        key = (r["excel_row"], r["col_letter"])
        prior = latest.get(key)
        if prior is None or _rank(r) > _rank(prior):
            latest[key] = r
