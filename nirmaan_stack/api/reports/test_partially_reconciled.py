...
⚠️ THE DATE TESTED IS THE BANK LINE'S, NOT THE RECORD'S, and that is the whole point of the range
tests below. These records carry no `payment_date` at all, so there is nothing on them a report's
date range could test -- see `partially_reconciled.py` for why the line's date is the right one.
"""
