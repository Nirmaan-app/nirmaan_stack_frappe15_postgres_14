...
def _make_project(name_suffix, tendering_status, status=None):
    ...
    empty; for Won projects it defaults to "Won".
    """
    if status is None:
        status = "" if tendering_status != "Won" else "Won"
    ...
    # we insert at "Won" and flip with frappe.db.set_value — matches the
...
class TestTenderingGuard(FrappeTestCase):
    ...
    WON_EXECUTION_STATUSES = [
        "Won",       # initial execution stage (v3.1: Won-as-initial)
        "Created",   # deprecated legacy fallback (pre-v3.1 rows)
        "WIP",
        "Completed",
        "Halted",
        "Handover",
