...
| **Bulk Unreconcile — several Settled lines at once** (#1317; read side #1319) | `api/outflow_import/bulk_unreconcile.py` — a thin loop (B4): `get_bulk_unreconcile_plan` calls `unreconcile.plan_of_line` per line (the SAME function `get_unreconcile_plan` wraps), cap `BULK_UNRECONCILE_LIMIT` = 50. Screen: `frontend/.../outflow-import/tickMode.ts` (which lines are tickable — open OR Settled, never both), `bulkUnreconcileView.ts` (the toolbar button, the check-step summary) | describe a line in the bulk check step differently from the one-line dialog, or put a decision rule in the bulk module. The per-line write it will call is `unreconcile.unreconcile_line` (#1318). ⚠️ The tick-mode rule lives ONLY in `tickMode.tickRules`; the page and table read its answer |
...
| **Who may Unreconcile or Reverse** (#1273, ADR-0022) | `api/outflow_import/permissions.require_outflow_undo_access` (`OUTFLOW_UNDO_PROFILES`: Admin + Accountant Lead, plus `Administrator`) — LAYERED on the module check. Frontend mirror `outflowImportStatus.canUndoOutflow`, pinned by `outflowUndoAccessParity.test.ts` | gate an undo action on the module check alone, or spell the profile set a second time. Callers: `expenses.reverse_allocation`, `unreconcile.get_unreconcile_plan`, `unreconcile.unreconcile_row` (checked at both the wrapper and the write) |
| **Who may Skip or Unskip** (ADR-0022 Amendment E, owner 2026-09-24) | the MODULE check, `permissions.require_outflow_access` — Accountant, Accountant Lead, Admin. Frontend: `canSkipByHand(row)` takes no role; the Skipped popup always shows Unskip | put Skip / Unskip back behind the undo check without an owner ruling — they move no recorded money, which is why they sit outside it |
