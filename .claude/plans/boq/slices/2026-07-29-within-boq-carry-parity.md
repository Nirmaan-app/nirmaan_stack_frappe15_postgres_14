...
| 2 | `frontend/CLAUDE.md` | carried-verdict cue *"Its one input is `SheetCategoryRow.carried_from_boq`"* | ❌ **STILL FALSE — hook-denied.** See F9: the correction needs ~+150 B minimum and there is NO prunable content contiguous with it (both carry bullets verified 0/30 duplicated anywhere in the repo), so no net-shrinking edit can carry it. Wording is drafted: three inputs (`carried_from_boq`, `carried_from_version`, server-derived `carried_from_other_boq`); tooltip reads "carried from Version N" within a BoQ vs "carried from BOQ-…" across one; **R16 — decided SERVER-SIDE** in `get_sheet_categories_resolved`; the STATE still keys on `carried_from_boq` alone. |
